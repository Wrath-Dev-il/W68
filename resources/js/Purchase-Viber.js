...
const statusKeys = ['entry', 'to_shipped', 'arrived', 'not_arrived', 'partial'];
...
function ensureViberColorStyles() {
    ...
    style.textContent = `
        ...
        .viber-row-shared input { background:#172554 !important; color:#fde047 !important; border-color:rgba(253,224,71,.35) !important; }
        ...
        .viber-row-rollback input { background:#fde047 !important; color:#4a0712 !important; border-color:#4a0712 !important; }
...
function getFilteredEntryItems(items) {
    ...
    Object.keys(colFilters).forEach(key => {
        ...
        if (val) {
            data = data.filter(e => String(e[key] || '').toLowerCase().includes(val));
        }
    });
        ...
        html += `<tr class="hover:bg-slate-50/50 transition-colors">
            ...
            <td class="py-3 px-4 text-center">
                <div class="inline-flex items-center gap-1.5">
                    ...
                    <button onclick="event.stopPropagation();printToShippedSupplierItems(${l.id})" class="px-3 py-1.5 bg-white border border-maroon text-maroon text-[10px] font-bold rounded-lg hover:bg-maroon-50 transition-all">Print</button>
                </div>
            </td>
        </tr>`;
    });
...
function renderToShippedItems() {
    ...
    if (addBtnArea) {
        addBtnArea.classList.toggle('hidden', VIBER_READ_ONLY || toShippedTab !== 'ready');
    }

    const items = toShippedTab === 'ready' ? toShippedReadyItems : toShippedShippedItems;

    if (items.length === 0) {
        const msg = toShippedTab === 'ready' ? 'No items ready to ship.' : 'No shipped items yet.';
        tbody.innerHTML = `<tr><td colspan="9" class="py-8 text-center text-sm text-slate-400">${msg}</td></tr>`;
        return;
    }
    let html = '';
    items.forEach(item => {
        const isClickable = !VIBER_READ_ONLY && toShippedTab === 'ready';
        ...
        html += `<tr onclick="${clickHandler}" class="transition-colors ${rowClass} ${bgClass} ${rollbackClass} ${sharedClass}">
            ...
            <td class="py-3 px-4 ${textClass}">${item.ordered_date || '—'}</td>
        </tr>`;
    });
    tbody.innerHTML = html;
}
...
window.addToNote = async function() {
    ...
    try {
        ...
        const json = await resp.json();
        if (json.success) {
            window.location.href = window.viberRoutes.purchaseNote + '?from_viber=1';
        } else {
...
function renderSupplierTabs() {
    ...
    lists.forEach(l => {
        ...
        html += `<div class="supplier-tab-wrapper inline-flex items-center gap-0 ${isActive ? '' : 'group'}">
            <button onclick="window.selectSupplierTab(${l.id})" style="${getSharedSupplierTabStyle(l, isActive)}" class="supplier-tab inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold ${VIBER_READ_ONLY ? 'rounded-lg' : 'rounded-l-lg'} border transition-all ${isActive ? 'bg-yellow-400 text-maroon border-yellow-500 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-maroon/30 hover:text-maroon'}">
            ...
            ${VIBER_READ_ONLY ? '' : `<button onclick="event.stopPropagation();openDeleteSupplierModal(${l.id})" class="inline-flex items-center justify-center px-2 py-1.5 text-xs font-bold rounded-r-lg border-l-0 border transition-all ${isActive ? 'bg-yellow-400 text-maroon border-yellow-500 hover:text-white hover:bg-red-600 hover:border-red-600' : 'bg-white text-slate-400 border-slate-200 hover:text-red-500 hover:border-red-300 group-hover:opacity-100 opacity-0'}">
...
async function renderTable() {
    ...
    if (pageData.length === 0) {
        ...
        if (data.length === 0 && items.length === 0) {
            ...
            tbody.innerHTML = `<tr><td colspan="${VIBER_READ_ONLY ? 10 : 11}" class="py-16 text-center text-slate-400">
    ...
    pageData.forEach(e => {
        ...
        const canTransferEntryItem = !VIBER_READ_ONLY && !hasPn;
        ...
        html += `<tr ${rowClick} data-product-id="${e.product_id || ''}" data-item-code="${escapePrintValue(e.item_code || '')}" data-item-id="${e.id}" class="hover:bg-slate-50 transition-colors border-b border-slate-100 ${rowCursor} ${rowClass} ${isTransferSelected ? 'viber-row-transfer-selected' : ''}">
            ...
            <td class="py-3 px-4 text-xs text-right font-semibold text-slate-800">
                ${VIBER_READ_ONLY ? formatCurrencyValue(e.new_cost, e.currency_code) : `<input type="number" step="0.01" min="0" value="${parseFloat(e.new_cost || 0).toFixed(2)}" data-field="new_cost" data-item-id="${e.id}" onclick="event.stopPropagation()" onchange="onInlineFieldChange(this, ${e.id})" class="inline-edit-input w-20 px-1.5 py-1 text-xs text-right border border-transparent hover:border-slate-200 focus:border-maroon/40 focus:ring-1 focus:ring-maroon/20 rounded-lg outline-none transition-all">
                <button onclick="saveInlineItem(${e.id})" id="inline-save-${e.id}" class="inline-save-btn hidden ml-1 px-2 py-1 bg-maroon text-white text-[9px] font-bold rounded-lg hover:bg-maroon-800 transition-all shadow-sm">Save</button>`}
            </td>
            <td class="py-3 px-4 text-xs text-center font-semibold">
                ${VIBER_READ_ONLY ? (e.order_qty ?? 0) : `<input type="number" min="0" value="${e.order_qty || 0}" data-field="order_qty" data-item-id="${e.id}" onclick="event.stopPropagation()" onchange="onInlineFieldChange(this, ${e.id})" class="inline-edit-input w-14 px-1.5 py-1 text-xs text-center border border-transparent hover:border-slate-200 focus:border-maroon/40 focus:ring-1 focus:ring-maroon/20 rounded-lg outline-none transition-all">`}
            </td>
            <td class="py-3 px-4 text-xs text-slate-600">${e.oum_unit || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-600">${e.ordered_date || ''}</td>
            <td class="py-3 px-4 text-xs text-slate-500 max-w-[140px]">
                ${VIBER_READ_ONLY ? escapePrintValue(e.remarks || '—') : `<input type="text" value="${e.remarks || ''}" data-field="remarks" data-item-id="${e.id}" onclick="event.stopPropagation()" onchange="onInlineFieldChange(this, ${e.id})" class="inline-edit-input w-full px-1.5 py-1 text-xs border border-transparent hover:border-slate-200 focus:border-maroon/40 focus:ring-1 focus:ring-maroon/20 rounded-lg outline-none transition-all" placeholder="—">`}
            </td>
            ${VIBER_READ_ONLY ? '' : `<td class="py-3 px-4 text-center">
                <button onclick="event.stopPropagation();openDeleteItemModal(${e.id})" class="inline-flex items-center justify-center p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all" title="Delete item">
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .print-table td:nth-child(1) { width: 12%; }
        .print-table th:nth-child(2),
        .print-table td:nth-child(2) { width: 14%; }
        .print-table th:nth-child(3),
        .print-table td:nth-child(3) { width: 14%; }
        .print-table th:nth-child(4),
        .print-table td:nth-child(4) { width: 13%; }
...
window.openAddItems = function() {
    ...
    modalStep = 1;
    modalSearchQuery = '';
    modalColumnFilters = { item_code: '', part_number: '', description: '', application: '' };
    modalCurrentPage = 1;
    modalProductsCache = [];
        ...
        if (json.data.length === 0) {
            ...
            json.data.forEach(i => {
                ...
                html += `<tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
                    ...
                    <td class="py-3 px-4 text-xs text-slate-600">${i.unit || '—'}</td>
                </tr>`;
            });
            tbody.innerHTML = html;
        }
...
function renderModalStep2() {
    ...
    modalSelectedItems.forEach((item, idx) => {
        html += `<tr class="border-b border-slate-100">
            ...
            <td class="py-3 px-4 text-xs text-slate-600">${item.unit || '—'}</td>
            ...
            <td class="py-3 px-4"><textarea onchange="updateModalItemField(${idx}, 'remarks', this.value)" rows="1" class="w-24 px-2 py-1.5 text-xs border border-slate-200 rounded-lg resize-none focus:ring-2 focus:ring-maroon/20 outline-none">${item.remarks || ''}</textarea></td>
        </tr>`;
    });
    tbody.innerHTML = html;
}
...
function updateActionButtons() {
    ...
    if (VIBER_READ_ONLY) {
        ...
        const active = getActiveList();
        if (printBtn) printBtn.classList.toggle('hidden', state.activeDashboard !== 'entry' || !active);
        const searchParent = document.getElementById('viber-search-input')?.parentElement;
        if (searchParent) searchParent.style.display = state.activeDashboard === 'entry' ? '' : 'none';
        return;
    }
...
window.openDeleteSupplierModal = function(viberListId) {
    ...
    supplierToDeleteId = viberListId;
...
window.confirmDeleteSupplier = async function() {
    ...
    try {
        const url = window.viberRoutes.destroySupplier.replace(':id', supplierToDeleteId);
        const resp = await fetch(url, {
            method: 'DELETE',
        ...
        if (json.success) {
            ...
            alert(json.message || 'Failed to delete supplier tab.');
        }
    } catch (e) {
        alert('Error deleting supplier tab.');
    } finally {
        isDeleting = false;
        btn.disabled = false;
...
window.confirmDeleteItem = async function() {
    ...
    try {
        const active = getActiveList();
        const url = window.viberRoutes.destroyItem.replace(':id', itemToDeleteId);
        const resp = await fetch(url, {
            method: 'DELETE',
            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '' }
        });
        ...
        if (json.success) {
            ...
            if (active && active._cachedItems) {
                active._cachedItems = active._cachedItems.filter(i => i.id !== itemToDeleteId);
                active.item_count = active._cachedItems.length;
            }
            renderAll();
            fetchDashboardCounts();
        } else {
...
window.onInlineFieldChange = function(input, itemId) {
    if (VIBER_READ_ONLY) return;
    const saveBtn = document.getElementById('inline-save-' + itemId);
    if (saveBtn) saveBtn.classList.remove('hidden');
};

window.saveInlineItem = async function(itemId) {
    if (VIBER_READ_ONLY) return;
    if (_inlineSaving[itemId]) return;
    ...
    try {
        const url = window.viberRoutes.updateItem.replace(':id', itemId);
        const resp = await fetch(url, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '' },
        ...
        if (json.success) {
            ...
            // Update cached item if present
            const active = getActiveList();
            if (active && active._cachedItems) {
                const cached = active._cachedItems.find(i => i.id === itemId);
                if (cached) {
                    if (payload.new_cost !== undefined) cached.new_cost = payload.new_cost;
                    if (payload.order_qty !== undefined) cached.order_qty = payload.order_qty;
                    if (payload.remarks !== undefined) cached.remarks = payload.remarks;
                }
            }
        } else {
...
function renderArrivedTable() {
    const card = document.getElementById('viber-arrived-invoiced-card');
    ...
    let html = '';
    pageData.forEach(e => {
        html += `<tr class="hover:bg-emerald-50/50 transition-colors border-b border-slate-100 ${e.is_shared_item ? 'viber-row-shared' : ''}">
            <td class="py-3 px-4 text-xs font-mono text-emerald-800 font-semibold">${e.item_code || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-700">${e.part_no || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-700 max-w-[200px] truncate" title="${e.description || ''}">${e.description || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-600">${e.application || '—'}</td>
            <td class="py-3 px-4 text-xs text-right text-slate-600">${formatCurrencyValue(e.last_cost, e.currency_code)}</td>
            <td class="py-3 px-4 text-xs text-right font-semibold text-slate-800">${formatCurrencyValue(e.new_cost, e.currency_code)}</td>
            <td class="py-3 px-4 text-xs text-center font-semibold text-slate-700">${e.order_qty != null ? parseFloat(e.order_qty) : '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-600">${e.oum_unit || '—'}</td>
            <td class="py-3 px-4 text-xs text-center font-semibold ${e.is_partial_arrived ? 'text-amber-700' : 'text-slate-700'}">${e.actual_qty != null ? parseFloat(e.actual_qty) : '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-600">${e.ordered_date || '—'}</td>
            <td class="py-3 px-4 text-xs font-mono text-slate-600">${e.po_number || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-600">${e.po_date || '—'}</td>
        </tr>`;
    });
    tbody.innerHTML = html;
    if (typeof lucide !== 'undefined') setTimeout(() => lucide.createIcons(), 0);
    ...
    try {
        ...
        if (json.success) {
            ...
            if (notArrivedSuppliers.length > 0) {
                notArrivedActiveListId = notArrivedSuppliers[0].id;
            }
            renderAll();
            fetchDashboardCounts();
        }
...
function renderNotArrivedTable() {
    ...
    let html = '';
    pageData.forEach(e => {
        html += `<tr class="hover:bg-amber-50/50 transition-colors border-b border-slate-100 ${e.is_shared_item ? 'viber-row-shared' : ''}">
            <td class="py-3 px-4 text-xs font-mono text-amber-800 font-semibold">${e.item_code || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-700">${e.part_no || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-700 max-w-[200px] truncate" title="${e.description || ''}">${e.description || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-600">${e.application || '—'}</td>
            <td class="py-3 px-4 text-xs text-right text-slate-600">${formatCurrencyValue(e.last_cost, e.currency_code)}</td>
            ...
            <td class="py-3 px-4 text-xs font-mono text-slate-600">${e.po_number || '—'}</td>
            <td class="py-3 px-4 text-xs text-slate-600">${e.po_date || '—'}</td>
            <td class="py-3 px-4 text-xs text-center font-semibold ${e.actual_qty < e.order_qty ? 'text-red-600' : 'text-slate-700'}">${e.actual_qty != null ? e.actual_qty : '—'}</td>
        </tr>`;
    });
    tbody.innerHTML = html;
    if (typeof lucide !== 'undefined') setTimeout(() => lucide.createIcons(), 0);
