...
async function fetchForwarders() {
    try {
        const response = await fetch(FORWARDER_ENDPOINTS.data, {
            headers: {
...
function renderForwarderTable() {
    ...
    tbody.innerHTML = filteredForwarders.map((fw) => {
...
const debouncedApplyForwarderFilters = debounce((render = true) => {
    const code = document.getElementById('col-search-code')?.value.toLowerCase() || '';
    const name = document.getElementById('col-search-name')?.value.toLowerCase() || '';
    const address = document.getElementById('col-search-address')?.value.toLowerCase() || '';
    const contact = document.getElementById('col-search-contact')?.value.toLowerCase() || '';
    const person = document.getElementById('col-search-person')?.value.toLowerCase() || '';

    filteredForwarders = forwarders.filter(fw =>
        safeString(fw.code).toLowerCase().includes(code) &&
        safeString(fw.name).toLowerCase().includes(name) &&
        safeString(fw.address).toLowerCase().includes(address) &&
        safeString(fw.contactNo).toLowerCase().includes(contact) &&
        safeString(fw.contactPerson).toLowerCase().includes(person)
    );

    if (render) renderForwarderTable();
    else renderForwarderTable();
}, 300);

function applyForwarderFilters(render = true) {
    debouncedApplyForwarderFilters(render);
}
...
window.saveForwarder = function(event) {
    ...
    showConfirmModal(title, message, async () => {
        ...
        try {
            ...
            resetForwarderFormState();
            showSuccessModal(isEdit ? 'Updated!' : 'Registered!', isEdit ? 'Forwarder record saved.' : 'Forwarder added successfully.');
        } catch (error) {
            console.error(isEdit ? 'Error updating forwarder:' : 'Error registering forwarder:', error);
            alert(error.message || (isEdit ? 'Failed to update forwarder' : 'Failed to register forwarder'));
        }
    });
}
