...
function renderPurchaseNoteItems(items) {
    ...
    tbody.innerHTML = items.map(item => `
        <tr>
            ...
            <td class="p-3 px-4 text-center">
                <input type="text" value="${item.unit || ''}" oninput="updateCalculations()" class="item-unit w-20 px-2 py-1.5 border border-slate-200 rounded text-center focus:ring-1 focus:ring-maroon/20" maxlength="50">
            </td>
            <td class="p-3 px-4">
...
function populatePrintLayout() {
    ...
    if (dateEl) {
        const selectedDate = document.getElementById('proceed-transaction-date')?.value;
        const dateValue = selectedDate ? new Date(`${selectedDate}T00:00:00`) : new Date();
        dateEl.textContent = dateValue.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
...
async function finalizePO() {
    ...
    try {
        ...
        rows.forEach(row => {
            ...
            const unit = String(row.querySelector('.item-unit')?.value || '').trim();
        ...
        const payload = {
            ...
            date: document.getElementById('proceed-transaction-date')?.value || new Date().toLocaleDateString('en-CA'),
...
window.openEditHistoryModal = async function(poId) {
    try {
        ...
        if (editDateInput) editDateInput.value = result.note?.date || new Date().toLocaleDateString('en-CA');
...
function populateEditPrintLayout() {
    ...
    if (dateEl) {
        const selectedDate = document.getElementById('edit-transaction-date')?.value;
        const dateValue = selectedDate ? new Date(`${selectedDate}T00:00:00`) : new Date();
        dateEl.textContent = dateValue.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
    }
...
window.confirmSaveEdit = async function() {
    ...
    try {
        ...
        const payload = {
            ...
            date: document.getElementById('edit-transaction-date')?.value || window.editState.originalData?.date || new Date().toLocaleDateString('en-CA'),
