...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/developer_control_panel.css') }}?v={{ time() }}">
    <script src="{{ asset('js/developer_control_panel.js') }}?v={{ time() }}" defer></script>
    <script>
        ...
        window.developerInventoryRoutes = {
            ...
            syncAll: "{{ route('developer.inventory-control.stock-mismatches.sync-all') }}",
            autoSyncStatus: "{{ route('developer.inventory-control.auto-sync-status') }}",
            purchaseLedgerMismatches: "{{ route('developer.inventory-control.purchase-ledger-mismatches') }}",
            purchaseLedgerSyncBase: "{{ url('/developer/purchase-ledger-mismatches') }}",
            purchaseLedgerSyncAll: "{{ route('developer.inventory-control.purchase-ledger-mismatches.sync-all') }}",
            purchaseReturnLedgerMismatches: "{{ route('developer.inventory-control.purchase-return-ledger-mismatches') }}",
            purchaseReturnLedgerSyncBase: "{{ url('/developer/purchase-return-ledger-mismatches') }}",
            purchaseReturnLedgerSyncAll: "{{ route('developer.inventory-control.purchase-return-ledger-mismatches.sync-all') }}",
            products: "{{ route('developer.inventory-control.products') }}",
            ledgerEntries: "{{ route('developer.inventory-control.ledger-entries') }}",
            ledgerStore: "{{ route('developer.inventory-control.ledger-store') }}",
            ledgerBase: "{{ url('/developer/product-ledgers') }}"
        };
    </script>
</head>
<body class="developer-shell min-h-screen bg-slate-100 text-slate-900" data-theme="{{ $activeTheme ?? 'default' }}" data-theme-now-ms="{{ $themeNowMs ?? now('Asia/Manila')->getTimestampMs() }}">
    ...
    <div class="min-h-screen">
            ...
            <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <h1 class="truncate text-xl font-extrabold text-slate-950 sm:text-2xl">{{ $fullName ?: 'Developer' }}</h1>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ url('/logout') }}" class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:border-[#4A0A15] hover:text-[#4A0A15]">
        ...
        <main class="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
            <section class="grid grid-cols-1 gap-5 md:grid-cols-3">
                <article class="developer-stat-card">
                    <div>
                        ...
                        <strong class="mt-2 block text-3xl font-extrabold tracking-tight text-slate-900">{{ number_format($stats['total_data'] ?? 0) }}</strong>
                ...
                <article class="developer-stat-card">
                    <div>
                        ...
                        <strong class="mt-2 block text-3xl font-extrabold tracking-tight text-slate-900">{{ number_format($stats['total_database'] ?? 0) }}</strong>
                ...
                <article class="developer-stat-card">
                    <div>
                        ...
                        <strong class="mt-2 block text-3xl font-extrabold tracking-tight text-slate-900">{{ number_format($stats['suspicious_attempts'] ?? 0) }}</strong>
            ...
            <section class="grid grid-cols-1 gap-6 xl:grid-cols-[1.25fr_0.75fr]">
                <div class="developer-panel">
                    <div class="developer-panel-heading">
                        <div>
                            <h2>Module Data</h2>
                            <p>{{ number_format($stats['total_data'] ?? 0) }} active records</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                        <div class="developer-mini-stat">
                            <span>Logins</span>
                            <strong>{{ number_format($stats['logins'] ?? 0) }}</strong>
                        </div>
                        <div class="developer-mini-stat">
                            <span>Products</span>
                            <strong id="developerProductCount">{{ number_format($stats['products'] ?? 0) }}</strong>
                        </div>
                        <div class="developer-mini-stat">
                            <span>Suppliers</span>
                            <strong>{{ number_format($stats['suppliers'] ?? 0) }}</strong>
                        </div>
                        <div class="developer-mini-stat">
                            <span>Customers</span>
                            <strong>{{ number_format($stats['customers'] ?? 0) }}</strong>
                        </div>
                        <div class="developer-mini-stat">
                            <span>Forwarders</span>
                            <strong>{{ number_format($stats['forwarders'] ?? 0) }}</strong>
                        </div>
                    </div>
                </div>
                ...
                <div class="developer-panel">
                    <div class="developer-panel-heading">
                        <div>
                            <h2>Data-ups</h2>
                            <p>6:00 PM {{ $backup['timezone'] ?? 'Asia/Manila' }}</p>
                        </div>
                        <button
                            type="button"
                            id="runDataUpBtn"
                            class="developer-secondary-btn"
                            data-run-url="{{ route('developer.data-ups.run') }}"
                        >
                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="space-y-3">
                        <div class="developer-backup-row">
                            ...
                            <strong>{{ $backup['next_at'] ?? 'Today 06:00 PM' }}</strong>
                        </div>
                        <div class="developer-backup-row">
                            <span>Stored backups</span>
                            <strong id="backupCount">{{ number_format($backup['count'] ?? 0) }}</strong>
                        </div>
                        <div class="developer-backup-row">
                            <span>Retention</span>
                            <strong>{{ $backup['retention_days'] ?? 7 }} days</strong>
                        </div>
                        <div class="developer-backup-row">
                            <span>Latest</span>
                            <strong id="latestBackup">{{ $backup['latest'] ?? 'None yet' }}</strong>
                        </div>
                    </div>
                    ...
                    <div class="developer-download-grid">
                        <button type="button" class="developer-download-btn" data-download-url="{{ route('developer.data-ups.download', ['type' => 'masterlist']) }}">
                        ...
                        <button type="button" class="developer-download-btn" data-download-url="{{ route('developer.data-ups.download', ['type' => 'purchase']) }}">
                            <span class="developer-download-icon">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                                    <polyline points="7 10 12 15 17 10"></polyline>
                        ...
                        <button type="button" class="developer-download-btn" data-download-url="{{ route('developer.data-ups.download', ['type' => 'sales']) }}">
                            <span class="developer-download-icon">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                                    <polyline points="7 10 12 15 17 10"></polyline>
                        ...
                        <button type="button" class="developer-download-btn" data-download-url="{{ route('developer.data-ups.download', ['type' => 'accounting']) }}">
                            <span class="developer-download-icon">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                                    <polyline points="7 10 12 15 17 10"></polyline>
                        ...
                        <button type="button" class="developer-download-btn" data-download-url="{{ route('developer.data-ups.download', ['type' => 'ledger']) }}">
                            <span class="developer-download-icon">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                                    <polyline points="7 10 12 15 17 10"></polyline>
                        ...
                        <button type="button" class="developer-download-all-btn" data-download-url="{{ route('developer.data-ups.download-all') }}">
                            <span class="developer-download-icon">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                                    <polyline points="7 10 12 15 17 10"></polyline>
            ...
            <section class="developer-panel">
                ...
                <div class="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6" data-theme-save-url="{{ route('developer.system-theme.save') }}" data-manual-default-window="{{ $themeState['manual_default_window'] ?? '' }}" data-last-auto-window="{{ $themeState['last_auto_window'] ?? '' }}">
                    ...
                    <button type="button" class="developer-theme-option" data-theme-option="christmas">
                        ...
                        <span class="developer-theme-copy">
                            ...
                            <small>Event Date: {{ $christmasCountdown['date_label'] ?? 'Dec 25, 2026' }}</small>
                            <small>Theme Starts: {{ $christmasCountdown['theme_start_label'] ?? 'Dec 10, 2026' }}</small>
                            <small>Theme Ends: {{ $christmasCountdown['theme_end_label'] ?? 'Dec 29, 2026' }}</small>
                            <small class="theme-countdown" data-theme-countdown="christmas" data-target="{{ $christmasCountdown['target_iso'] ?? '2026-12-25T00:00:00+08:00' }}" data-auto-start="{{ $christmasCountdown['theme_start_iso'] ?? '2026-12-10T00:00:00+08:00' }}" data-theme-end="{{ $christmasCountdown['theme_end_iso'] ?? '2026-12-29T23:59:59+08:00' }}" data-target-ms="{{ $christmasCountdown['target_ms'] ?? '' }}" data-auto-start-ms="{{ $christmasCountdown['theme_start_ms'] ?? '' }}" data-theme-end-ms="{{ $christmasCountdown['theme_end_ms'] ?? '' }}" data-priority="{{ $christmasCountdown['priority'] ?? 3 }}" data-window-key="{{ $christmasCountdown['window_key'] ?? 'christmas:2026-12-25' }}">Remaining Until Event: --h --m --s</small>
                        </span>
                    </button>
                    ...
                    <button type="button" class="developer-theme-option" data-theme-option="holy-week">
                        ...
                        <span class="developer-theme-copy">
                            ...
                            <small>Event Date: {{ $holyWeekCountdown['date_label'] ?? 'Mar 26, 2027' }}</small>
                            <small>Theme Starts: {{ $holyWeekCountdown['theme_start_label'] ?? 'Mar 11, 2027' }}</small>
                            <small>Theme Ends: {{ $holyWeekCountdown['theme_end_label'] ?? 'Mar 31, 2027' }}</small>
                            <small class="theme-countdown" data-theme-countdown="holy-week" data-target="{{ $holyWeekCountdown['target_iso'] ?? '2027-03-26T00:00:00+08:00' }}" data-auto-start="{{ $holyWeekCountdown['theme_start_iso'] ?? '2027-03-11T00:00:00+08:00' }}" data-theme-end="{{ $holyWeekCountdown['theme_end_iso'] ?? '2027-03-31T23:59:59+08:00' }}" data-target-ms="{{ $holyWeekCountdown['target_ms'] ?? '' }}" data-auto-start-ms="{{ $holyWeekCountdown['theme_start_ms'] ?? '' }}" data-theme-end-ms="{{ $holyWeekCountdown['theme_end_ms'] ?? '' }}" data-priority="{{ $holyWeekCountdown['priority'] ?? 4 }}" data-window-key="{{ $holyWeekCountdown['window_key'] ?? 'holy-week:2027-03-26' }}">Remaining Until Event: --h --m --s</small>
                        </span>
                    </button>
                    ...
                    <button type="button" class="developer-theme-option" data-theme-option="halloween">
                        ...
                        <span class="developer-theme-copy">
                            ...
                            <small>Event Date: {{ $halloweenCountdown['date_label'] ?? 'Oct 31, 2026' }}</small>
                            <small>Theme Starts: {{ $halloweenCountdown['theme_start_label'] ?? 'Oct 16, 2026' }}</small>
                            <small>Theme Ends: {{ $halloweenCountdown['theme_end_label'] ?? 'Nov 05, 2026' }}</small>
                            <small class="theme-countdown" data-theme-countdown="halloween" data-target="{{ $halloweenCountdown['target_iso'] ?? '2026-10-31T00:00:00+08:00' }}" data-auto-start="{{ $halloweenCountdown['theme_start_iso'] ?? '2026-10-16T00:00:00+08:00' }}" data-theme-end="{{ $halloweenCountdown['theme_end_iso'] ?? '2026-11-05T23:59:59+08:00' }}" data-target-ms="{{ $halloweenCountdown['target_ms'] ?? '' }}" data-auto-start-ms="{{ $halloweenCountdown['theme_start_ms'] ?? '' }}" data-theme-end-ms="{{ $halloweenCountdown['theme_end_ms'] ?? '' }}" data-priority="{{ $halloweenCountdown['priority'] ?? 5 }}" data-window-key="{{ $halloweenCountdown['window_key'] ?? 'halloween:2026-10-31' }}">Remaining Until Event: --h --m --s</small>
                        </span>
                    </button>
                    ...
                    <button type="button" class="developer-theme-option" data-theme-option="new-year">
                        ...
                        <span class="developer-theme-copy">
                            ...
                            <small>Event Date: {{ $newYearCountdown['date_label'] ?? 'Jan 01, 2027' }}</small>
                            <small>Theme Starts: {{ $newYearCountdown['theme_start_label'] ?? 'Dec 30, 2026' }}</small>
                            <small>Theme Ends: {{ $newYearCountdown['theme_end_label'] ?? 'Jan 10, 2027' }}</small>
                            <small class="theme-countdown" data-theme-countdown="new-year" data-target="{{ $newYearCountdown['target_iso'] ?? '2027-01-01T00:00:00+08:00' }}" data-auto-start="{{ $newYearCountdown['theme_start_iso'] ?? '2026-12-30T00:00:00+08:00' }}" data-theme-end="{{ $newYearCountdown['theme_end_iso'] ?? '2027-01-10T23:59:59+08:00' }}" data-target-ms="{{ $newYearCountdown['target_ms'] ?? '' }}" data-auto-start-ms="{{ $newYearCountdown['theme_start_ms'] ?? '' }}" data-theme-end-ms="{{ $newYearCountdown['theme_end_ms'] ?? '' }}" data-priority="{{ $newYearCountdown['priority'] ?? 1 }}" data-window-key="{{ $newYearCountdown['window_key'] ?? 'new-year:2027-01-01' }}">Remaining Until Event: --h --m --s</small>
                        </span>
                    </button>
                    ...
                    <button type="button" class="developer-theme-option" data-theme-option="chinese-new-year">
                        ...
                        <span class="developer-theme-copy">
                            ...
                            <small>Event Date: {{ $chineseNewYearCountdown['date_label'] ?? 'Feb 06, 2027' }}</small>
                            <small>Theme Starts: {{ $chineseNewYearCountdown['theme_start_label'] ?? 'Jan 22, 2027' }}</small>
                            <small>Theme Ends: {{ $chineseNewYearCountdown['theme_end_label'] ?? 'Feb 11, 2027' }}</small>
                            <small class="theme-countdown" data-theme-countdown="chinese-new-year" data-target="{{ $chineseNewYearCountdown['target_iso'] ?? '2027-02-06T00:00:00+08:00' }}" data-auto-start="{{ $chineseNewYearCountdown['theme_start_iso'] ?? '2027-01-22T00:00:00+08:00' }}" data-theme-end="{{ $chineseNewYearCountdown['theme_end_iso'] ?? '2027-02-11T23:59:59+08:00' }}" data-target-ms="{{ $chineseNewYearCountdown['target_ms'] ?? '' }}" data-auto-start-ms="{{ $chineseNewYearCountdown['theme_start_ms'] ?? '' }}" data-theme-end-ms="{{ $chineseNewYearCountdown['theme_end_ms'] ?? '' }}" data-priority="{{ $chineseNewYearCountdown['priority'] ?? 2 }}" data-window-key="{{ $chineseNewYearCountdown['window_key'] ?? 'chinese-new-year:2027-02-06' }}">Remaining Until Event: --h --m --s</small>
                        </span>
                    </button>
                </div>
            </section>
    ...
    <div id="importModal" class="developer-modal hidden" aria-hidden="true">
        <div class="developer-modal-panel">
            ...
            <form id="importForm" class="space-y-5" method="POST" enctype="multipart/form-data" data-import-url="{{ route('developer.import') }}">
                <div>
