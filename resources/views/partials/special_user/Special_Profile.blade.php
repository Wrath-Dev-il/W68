                    ...
                    <div class="profile-avatar-card">
                        <div class="profile-avatar-wrap profile-avatar-wrap-main">
                            @if($profilePictureUrl)
                                <img id="admin-profile-picture-preview" src="{{ $profilePictureUrl }}" alt="Admin profile picture" class="profile-avatar-image">
                            @else
                                <div id="admin-profile-picture-fallback" class="profile-avatar-fallback">
                                    {{ strtoupper(substr($profileUser->User_First_Name ?? 'A', 0, 1) . substr($profileUser->User_Last_Name ?? 'D', 0, 1)) }}
                                </div>
                        ...
                        <div class="text-center">
                            <h3 id="admin-profile-display-name" class="text-lg font-black text-slate-800">
                                {{ trim(collect([$profileUser->User_First_Name, $profileUser->User_Middle_Name, $profileUser->User_Last_Name])->filter()->implode(' ')) }}
                            </h3>
                    ...
                    <form id="admin-profile-form" class="p-5 space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="profile-input-group">
                                ...
                                <input id="profile-account-type" type="text" class="profile-input-field profile-input-readonly" value="{{ $accountTypeLabel }}" readonly>
                            </div>
                            ...
                            <div class="profile-input-group">
                                ...
                                <input id="profile-user-id" type="text" class="profile-input-field profile-input-readonly" value="{{ $profileUser->User_ID }}" readonly>
                            </div>
                            ...
                            <div class="profile-input-group">
                                ...
                                <input id="profile-first-name" name="User_First_Name" type="text" class="profile-input-field" value="{{ $profileUser->User_First_Name }}" required>
                            </div>
                            ...
                            <div class="profile-input-group">
                                ...
                                <input id="profile-middle-name" name="User_Middle_Name" type="text" class="profile-input-field" value="{{ $profileUser->User_Middle_Name }}">
                            </div>
                            ...
                            <div class="profile-input-group md:col-span-2">
                                ...
                                <input id="profile-last-name" name="User_Last_Name" type="text" class="profile-input-field" value="{{ $profileUser->User_Last_Name }}" required>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="profile-picture-preview-card">
                            ...
                            <div class="profile-avatar-wrap profile-avatar-wrap-modal">
                                @if($profilePictureUrl)
                                    <img id="admin-profile-picture-modal-preview" src="{{ $profilePictureUrl }}" alt="Profile preview" class="profile-avatar-image">
                                @else
...
@push('scripts')
    <script>
        window.adminProfileData = {
            chartLabels: @json($loginGraphLabels),
            chartValues: @json($loginGraphData),
            profilePictureUrl: @json($profilePictureUrl),
        };
        window.adminProfileRoutes = {
            updateProfile: "{{ route('special.profile.update') }}",
            uploadPicture: "{{ route('special.profile.picture.update') }}"
        };
    </script>
