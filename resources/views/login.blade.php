...
<body>
  ...
  <div class="page-wrapper">
    ...
    <div class="right-side">
      ...
      <div class="form-content">
        ...
        <!-- Error Alert -->
        <div class="alert @if(session('error')) show @endif" id="alertBox" role="alert" aria-live="polite">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
          </svg>
          <span id="alertMsg">@if(session('error')) {{ session('error') }} @else Please fill in all fields. @endif</span>
        </div>
  ...
  @if($showDeveloperOtp || session('developer_otp_lockout'))
  <div id="developerOtpModal" class="modal-backdrop show">
    <form method="POST" action="{{ route('developer.otp.verify') }}" class="modal-card otp-modal-card" autocomplete="off">
      @csrf
      <div class="modal-icon">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
      ...
      @if(session('developer_otp_lockout'))
      ...
      @else
        @if($developerOtpEmail)
          <p class="modal-desc">OTP email: <span class="otp-email">{{ $developerOtpEmail }}</span></p>
        @endif

        @if(session('developer_otp_notice') || session('developer_otp_mail_pending'))
          <div class="otp-status otp-status-success" id="developerOtpStatus">
            {{ session('developer_otp_notice', 'Sending developer OTP to your email...') }}
          </div>
        @endif

        @if(session('developer_otp_error'))
          <div class="otp-status otp-status-error">{{ session('developer_otp_error') }}</div>
        @endif
  ...
  <script>
    ...
    @if(session('lockout'))
      window.lockoutSeconds = {{ session('seconds') }};
    @else
      window.lockoutSeconds = 0;
    @endif
    window.developerOtpSeconds = {{ $developerOtpSeconds ?? 0 }};
    window.developerOtpLockoutSeconds = {{ $developerOtpLockoutSeconds ?? 0 }};
    window.developerOtpMailPending = @json((bool) session('developer_otp_mail_pending'));
    window.developerOtpSendRoute = "{{ route('developer.otp.send') }}";
