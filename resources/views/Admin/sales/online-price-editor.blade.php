...
<div class="max-w-7xl mx-auto py-8 px-6 space-y-6">
    ...
    <input type="hidden" id="out-date1" value="{{ $outDate1 ?? '' }}">
    <input type="hidden" id="out-date2" value="{{ $outDate2 ?? '' }}">
    <input type="hidden" id="out-date3" value="{{ $outDate3 ?? '' }}">

    <form id="redirect-form" method="GET" action="{{ route('admin.sales-note.report-redirect') }}" style="display:none">
...
window.submitPrices = async function() {
    ...
    try {
        const res = await fetch('{{ route("admin.sales-note.online-prices.save") }}', {
            method: 'POST',
