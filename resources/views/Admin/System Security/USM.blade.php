@push('styles')
    <link rel="stylesheet" href="{{ asset('css/usm.css') }}?v={{ time() }}">
@endpush

@section('usm_content')
<script>
    try { window.dbUsers = JSON.parse(atob('{{ base64_encode(json_encode($dbUsers->toArray())) }}')); } catch(e) { window.dbUsers = []; }
    try { window.loggedInAdmin = JSON.parse(atob('{{ base64_encode(json_encode($user)) }}')); } catch(e) { window.loggedInAdmin = null; }
    window.usmOnlineUserIds = @json($onlineUserIds->values()->toArray());
    window.csrfToken = '{{ csrf_token() }}';
    window.usmRoutes = {
        create: '{{ route("admin.usm.create") }}',
        resetPassword: '{{ route("admin.usm.reset-password") }}',
        delete: '{{ route("admin.usm.delete") }}'
    };
</script>
...
@push('scripts')
    <script src="{{ asset('js/usm.js') }}?v={{ time() }}"></script>
@endpush
