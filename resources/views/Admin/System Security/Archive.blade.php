    ...
    <div id="archive-page" class="space-y-6" data-data-url="{{ route('admin.archived.data') }}" data-restore-url="{{ route('admin.archived.restore') }}">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-5">
            @foreach([
                ...
                <button type="button" class="archive-card bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between group hover:border-slate-200 transition-all hover:shadow-md hover:scale-[1.01] duration-200 overflow-hidden text-left" data-category="{{ $card['key'] }}">
                    ...
                    <div class="mt-5 grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                        @foreach([
                            ...
                            <div class="archive-guide-row">
                                <div class="font-black text-slate-800 uppercase tracking-widest text-[10px]">{{ $row[0] }}</div>
                                <div class="text-slate-500 font-semibold">{{ $row[1] }}</div>
                            </div>
                        @endforeach
                    </div>
