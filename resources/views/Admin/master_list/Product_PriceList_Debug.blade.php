{{--
    ⚠ TEMPORARY DEBUG VIEW — REMOVE AFTER PAGINATION IS FIXED ⚠
    File: resources/views/Admin/master_list/Product_PriceList_Debug.blade.php
    URL:  /admin/debug/pricelist-measure

    Renders the EXACT same print HTML as Product_PriceList.blade.php
    and uses JavaScript to measure actual browser-rendered heights
    of every .print-page div and every <tr> row.

    Does NOT modify any production behaviour.
--}}
<!DOCTYPE html>
<html lang="en">
<head>
...
{{-- ── Hidden render area: EXACT same HTML as Product_PriceList.blade.php ──────── --}}
{{-- PHP pagination logic (identical copy from production blade) --}}
@php
...
<div id="render-area">
    @foreach($printPages as $pageIndex => $pageData)
    <div class="print-page"
         data-page="{{ $pageIndex + 1 }}"
         data-php-h="{{ $pageData['finalH'] }}"
         data-cats="{{ count($pageData['descriptions']) }}">
        ...
        <table class="print-table">
            ...
            @foreach($pageData['items'] as $item)
                @if($item['isNewGroup'])
                <tr data-type="cat">
                    <th colspan="5" class="desc-header">{{ trim(strtoupper($item['product']->description ?? 'NO DESCRIPTION')) }}</th>
                </tr>
                @endif
                @php
                ...
                @endphp
                <tr data-type="{{ $item['isPos'] ? 'pos' : 'row' }}"
                    data-app="{{ e($_appText) }}"
                    data-pos="{{ e($_posText) }}"
                    data-code="{{ e($item['product']->product_code ?? '') }}"
                    data-app-len="{{ mb_strlen($_appText) }}"
                    data-pos-len="{{ mb_strlen($_posText) }}"
                    data-php-app-lines="{{ $item['phpAppLines'] ?? '?' }}"
                    data-php-pos-lines="{{ $item['phpPosLines'] ?? '?' }}"
                    data-php-h="{{ $item['phpH'] ?? '?' }}">
                    <td style="border:2px solid black;width:15%;">{{ $item['product']->product_code }}</td>
                    <td style="border:2px solid black;width:20%;font-family:monospace;">{{ $item['product']->part_number ?? '---' }}</td>
                    <td style="border:2px solid black;width:35%;">
                        <div data-role="app-div">{{ $item['product']->application ?? $item['product']->Application ?? '---' }}</div>
                        @php $pos = $item['product']->position ?? $item['product']->Position; @endphp
                        @if($pos)
                    ...
                    <td style="border:2px solid black;width:15%;text-transform:uppercase;">{{ $item['product']->category }}</td>
                    <td style="border:2px solid black;width:15%;" class="price-col">₱{{ number_format($item['product']->selling_price, 2) }}</td>
                </tr>
            @endforeach
            </tbody>
        </table>
        ...
        <div class="page-ftr" data-role="footer">
            ...
            <span style="font-family:'Arial Black',Arial,sans-serif;font-size:12px;font-weight:900;text-transform:uppercase;color:black;white-space:nowrap;">PAGE {{ $pageIndex + 1 }} OF {{ $totalPrintPages }}</span>
        </div>
    </div>
    @endforeach
</div>
...
function runMeasurement() {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
        ...
        push('  PHP pages    : {{ $totalPrintPages }}');
        ...
        pages.forEach(function(pg, pi) {
            ...
            push('───────────────────────────────────────────────────────────────────────────');
            push('  PAGE ' + (pi + 1) + ' OF {{ $totalPrintPages }}');
            push('───────────────────────────────────────────────────────────────────────────');
        ...
        push('  Expected Chrome physical pages: ' + ({{ $totalPrintPages }} + overflowCount));
