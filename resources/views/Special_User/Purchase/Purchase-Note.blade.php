...
<script>
    console.log('[DEBUG] App URL:', '{{ config("app.url") }}');
    console.log('[DEBUG] Asset URL:', '{{ asset("") }}');
    console.log('[DEBUG] Check Password Route:', '{{ url("/check-password") }}');

    window.purchaseRoutes = {
        fetch: '{{ route("special.purchase.note.data") }}',
        details: '{{ route("special.purchase.note.details", ["id" => ":id"]) }}',
        searchSuppliers: '{{ route("special.purchase.search-suppliers") }}',
        searchProducts: '{{ route("special.purchase.search-products") }}',
        store: '{{ route("special.purchase.note.store") }}',
        update: '{{ route("special.purchase.note.update", ["id" => ":id"]) }}',
        delete: '{{ route("special.purchase.note.delete", ["id" => ":id"]) }}',
        supplierItemConflicts: '{{ route("special.purchase.note.supplier-item-conflicts") }}',
        forgottenPartialItems: '{{ route("special.purchase.note.forgotten-partial-items", ["supplierCode" => ":supplierCode"]) }}',
        nextNumber: '{{ route("special.purchase.note.next-number") }}',
        print: '{{ route("special.purchase.note.print", ["id" => ":id"]) }}',
        verifyPasswordUrl: '{{ route("special.purchase.note.verify-password") }}',
        checkPasswordUrl: '{{ url("/check-password") }}',
        invoicesByNoteUrl: '{{ route("special.purchase-order.by-note", ["purchaseNoteNumber" => ":noteNumber"]) }}',
        invoiceDetailUrl: '{{ route("special.purchase-order.detail", ["poId" => ":poId"]) }}',
        purchaseOrderIndex: '{{ route("special.purchase-order") }}'
    };

    console.log('[DEBUG] Purchase Routes:', window.purchaseRoutes);

    @if(isset($viberData) && $viberData)
...
@push('scripts')
    <script src="{{ asset('js/Purchase-Note.js') }}?v={{ time() }}"></script>
@endpush
@endsection
