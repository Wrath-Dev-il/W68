...
<style>
    ...
    #product-tbody td:first-child { display: none !important; }
</style>
@endpush

@push('styles')
    <link rel="stylesheet" href="{{ asset('css/product_master_list.css') }}?v={{ time() }}">
@endpush

@section('prod_master_content')
<script>
    window.prodRoutes = {
        fetch: '{{ route("special.prod-master.data") }}',
        create: '{{ route("special.prod-master.create") }}',
        update: '{{ route("special.prod-master.update", ["id" => ":id"]) }}',
        delete: '{{ route("special.prod-master.delete", ["id" => ":id"]) }}',
       'ledger': '{{ route("special.prod-master.ledger", ["id" => ":id"]) }}',
        'select': '{{ route("special.prod-master.select") }}',
        'reorderLevel': '{{ route("special.prod-master.reorder-level") }}',
        'catalog': '{{ route("special.prod-master.catalog") }}',
        'priceList': '{{ route("special.prod-master.price-list") }}',
        'activeNotes': '{{ route("special.prod-master.active-notes", ["id" => ":id"]) }}',
        'priceCodesUpdate': '{{ route("special.prod-master.price-codes.update", ["id" => ":id"]) }}'
    };
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div onclick="switchTab('all')" class="cursor-pointer bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between transition-all duration-200 hover:shadow-md hover:border-slate-200 hover-card-trigger">
            <div class="space-y-1">
                ...
                <h3 id="card-count-total" class="text-2xl font-extrabold text-slate-800 tracking-tight">{{ $total_products ?? 0 }}</h3>
            </div>
        ...
        <div onclick="switchTab('newly')" class="cursor-pointer bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between transition-all duration-200 hover:shadow-md hover:border-slate-200 hover-card-trigger">
            <div class="space-y-1">
                ...
                <h3 id="card-count-newly" class="text-2xl font-extrabold text-slate-800 tracking-tight">{{ $newly_added ?? 0 }}</h3>
            </div>
            <div class="w-12 h-12 rounded-xl flex items-center justify-center bg-maroon-gradient shadow-lg flex-shrink-0 ml-4 border border-goldlining-500/20">
        ...
        <div onclick="switchTab('low')" class="cursor-pointer bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between transition-all duration-200 hover:shadow-md hover:border-slate-200 hover-card-trigger">
            <div class="space-y-1">
                ...
                <h3 id="card-count-low" class="text-2xl font-extrabold text-slate-800 tracking-tight text-red-600">{{ $low_stock ?? 0 }}</h3>
            </div>
            <div class="w-12 h-12 rounded-xl flex items-center justify-center bg-maroon-gradient shadow-lg flex-shrink-0 ml-4 border border-goldlining-500/20">
    ...
    <div id="product-gallery-modal" class="fixed inset-0 z-[900] overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen p-4 text-center sm:p-0">
            ...
            <div class="relative inline-block align-middle bg-transparent overflow-hidden transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                <!-- Close Button -->
                <button onclick="toggleModal('product-gallery-modal', false)" class="absolute top-4 right-4 z-[110] text-white/70 hover:text-white transition-colors p-2 bg-white/10 rounded-full hover:bg-white/20">
                ...
                <div id="gallery-thumbnails" class="mt-6 flex justify-center gap-3 overflow-x-auto py-2 px-4 custom-scrollbar">
                    <!-- Thumbnails go here -->
                </div>
            </div>
        </div>
...
@push('scripts')
    <script src="{{ asset('js/product_master_list.js') }}?v={{ time() }}"></script>
@endpush
