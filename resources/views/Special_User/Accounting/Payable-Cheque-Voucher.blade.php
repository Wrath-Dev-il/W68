...
@section('payable_cheque_voucher_content')
        ...
        <div class="relative bg-white w-full h-[95vh] max-w-[1400px] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div class="px-6 py-4 bg-maroon-800 border-b border-white/10">
                <div class="relative max-w-2xl mx-auto">
                    ...
                    @foreach ([1 => 'Supplier', 2 => 'Sudden Return', 3 => 'Invoices', 4 => 'Payment', 5 => 'Review'] as $step => $label)
                        <div id="pcv-stepper-item-{{ $step }}" class="pcv-stepper-item flex flex-col items-center gap-2">
                            <span id="pcv-step-dot-{{ $step }}" class="pcv-step-dot {{ $step === 1 ? 'active' : '' }}">{{ $step }}</span>
                            <span class="text-[9px] font-black uppercase tracking-widest text-white/70">{{ $label }}</span>
                        </div>
                    @endforeach
                    </div>
                </div>
                    ...
                    <div class="max-w-3xl space-y-5">
                        ...
                        <div>
                            ...
                            <div class="mt-2 grid grid-cols-1 sm:grid-cols-4 gap-3">
                                @foreach (['Gcash', 'Cash', 'Cheque', 'Bank Transfer'] as $method)
                                    <label class="flex items-center gap-3 border border-slate-200 rounded-xl p-4 cursor-pointer hover:border-maroon-200">
                                        <input name="pcv-payment-method" type="radio" value="{{ $method }}" class="accent-maroon-900" {{ $method === 'Gcash' ? 'checked' : '' }}>
...
@push('scripts')
    ...
    <script src="{{ asset('js/Payable-Cheque-Voucher-Enhancement.js') }}?v={{ time() }}"></script>
    <script>
    ...
    <script>
        window.pcvRoutes = {
            data: "{{ route('special.payable-cheque-voucher.data') }}",
            supplier: "{{ url('/special/accounting/payable-cheque-voucher/supplier') }}",
            process: "{{ route('special.payable-cheque-voucher.process') }}",
            returns: "{{ url('/special/accounting/payable-cheque-voucher/returns') }}",
            suddenReturns: "{{ url('/special/accounting/payable-cheque-voucher/sudden-returns') }}",
            history: "{{ url('/special/accounting/payable-cheque-voucher/history') }}",
            historyDetail: "{{ url('/special/accounting/payable-cheque-voucher/history') }}",
            show: "{{ url('/special/accounting/payable-cheque-voucher/show') }}",
            updateNew: "{{ url('/special/accounting/payable-cheque-voucher/update-new') }}",
            draftSave: "{{ url('/special/accounting/payable-cheque-voucher/draft/save') }}",
            draftGet: "{{ url('/special/accounting/payable-cheque-voucher/draft/get') }}",
            draftClear: "{{ url('/special/accounting/payable-cheque-voucher/draft/clear') }}",
            recommendations: "{{ url('/special/accounting/payable-cheque-voucher/supplier') }}"
        };
        ...
        (() => {
            ...
            async function openPrintModal(supplierId) {
                ...
                try {
                    ...
                    const res = await fetch(`{{ url('/special/accounting/payable-cheque-voucher/history/supplier') }}/${supplierId}/vouchers`);
                    const data = await res.json();
            ...
            window.pcvSubmitDeleteVoucher = async function() {
                ...
                try {
                    const res = await fetch(`{{ url('/special/accounting/payable-cheque-voucher/delete') }}/${deleteVoucherId}`, {
                        method: 'DELETE',
                        headers: {
...
@include('partials.special_user.special_sidebar_navbar')
