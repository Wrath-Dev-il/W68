...
function populateProceedReview() {
    ...
    document.getElementById('rev-p-date').textContent = note.order_date;
...
window.submitPrintReceipt = async function() {
    ...
    if (window._printSource === 'history') {
        try {
            ...
            date = note.order_date || note.date_issue || '';
        } catch (e) {
        ...
        netTotal = grossTotal - totalAddlDiscAmt;
        date = document.getElementById('rev-p-date')?.textContent || note.order_date || '';
    }
...
window.viewSalesOrderDetail = async function(id) {
    ...
    try {
        ...
        if (r.success) {
            ...
            document.getElementById('view-order-label').textContent = 'Transaction: ' + (order.invoice_no || order.sales_number);
            document.getElementById('view-h-date').textContent = order.date_issue || '---';
            document.getElementById('view-h-invoice').textContent = order.invoice_no || '---';
            ...
            document.getElementById('view-i-sn').textContent = order.sales_number || '---';
            document.getElementById('view-i-customer').textContent = order.customer_name || '---';
            document.getElementById('view-i-date').textContent = order.order_date || '---';
            document.getElementById('view-i-invoices').textContent = order.invoice_numbers || '---';
            ...
            if (order.items && order.items.length > 0) {
                tbody.innerHTML = order.items.map(item => `
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="p-4 font-bold text-maroon" style="min-width:150px;width:150px;white-space:normal;word-break:break-word;overflow-wrap:anywhere;">${item.product_code}</td>
                        <td class="p-4 text-slate-500" style="min-width:150px;width:150px;white-space:normal;word-break:break-word;overflow-wrap:anywhere;">${item.price_code || 'N/A'}</td>
                        <td class="p-4 text-slate-700">${item.description || ''}</td>
                        <td class="p-4 text-center font-bold">${item.quantity}</td>
                        <td class="p-4 text-center ${item.actual_qty !== item.quantity ? 'text-amber-600 font-bold' : 'text-slate-700'}">${item.actual_qty ?? item.quantity}</td>
                        <td class="p-4 text-center text-amber-600 font-bold">${item.additional_qty || 0}</td>
                        <td class="p-4 text-center uppercase text-slate-500">${item.oum}</td>
                        <td class="p-4 text-right">₱ ${parseFloat(item.unit_price).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                        <td class="p-4 text-center text-slate-400">${item.discount}%</td>
                        <td class="p-4 text-right font-bold text-maroon">₱ ${parseFloat(item.subtotal).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                        <td class="p-4 text-slate-500 italic text-[10px]">${item.particulars || '---'}</td>
                    </tr>
                `).join('');
                document.getElementById('view-order-count').textContent = order.items.length + ' items';
    ...
    } catch (e) { console.error('Error loading order detail:', e);
...
window.openOnlineReportEdit = async function(reportId) {
    try {
        ...
        window._onlineReportCounterParts = data.counter_parts || {};
        window._onlineReportAddedItems = {};
...
function addItemToOnlineReport(noteIdx, product, qty) {
    ...
    // Check duplicate
    const exists = window._onlineReportAddedItems[noteIdx].some(i => String(i.product_id) === String(product.id));
    if (exists) return;
...
window.removeSelectedOnlineProduct = function(noteIdx, productId) {
    ...
    const originalItems = (note.net_total == 0) ? [] : (note.items || []);
...
function syncOnlinePrices() {
    document.querySelectorAll('.or-price-input').forEach(input => {
        ...
        if (!isNaN(noteIdx) && !isNaN(itemIdx)) {
            if (_onlineReportNotes[noteIdx] && _onlineReportNotes[noteIdx].items[itemIdx]) {
                _onlineReportNotes[noteIdx].items[itemIdx].price_online = parseFloat(input.value) || 0;
            }
        }
    });
}
...
function syncOnlineQty() {
    document.querySelectorAll('.or-qty-input').forEach(input => {
        ...
        if (!isNaN(noteIdx) && !isNaN(itemIdx)) {
            ...
            if (_onlineReportNotes[noteIdx]) {
                if (_onlineReportNotes[noteIdx].items[itemIdx]) {
                    _onlineReportNotes[noteIdx].items[itemIdx].quantity = qty;
                }
                const addedItems = (window._onlineReportAddedItems && window._onlineReportAddedItems[noteIdx]) || [];
                const origCount = _onlineReportNotes[noteIdx].items && _onlineReportNotes[noteIdx].net_total != 0 ? _onlineReportNotes[noteIdx].items.length : 0;
                const addedIdx = itemIdx - origCount;
...
function renderOnlineReportItems() {
    ...
    _onlineReportNotes.forEach((note, noteIdx) => {
        ...
        const items = (note.net_total == 0) ? [] : (note.items || []);
        const addedItems = (window._onlineReportAddedItems && window._onlineReportAddedItems[noteIdx]) || [];
                    ...
                    '<tbody class="divide-y divide-slate-100 text-sm">' +
                        ...
                        items.map((item, itemIdx) => {
                            ...
                            return '<tr>' +
                                ...
                                '<td class="p-3 px-4 text-center text-slate-300">---</td>' +
                            '</tr>';
                        }).join('') +
    ...
    document.querySelectorAll('.or-qty-input').forEach(input => {
        input.addEventListener('input', function() {
            const noteIdx = parseInt(this.getAttribute('data-or-note-idx'));
            const itemIdx = parseInt(this.getAttribute('data-or-item-idx'));
            if (!isNaN(noteIdx) && !isNaN(itemIdx)) {
...
window.generateOnlineReceipt = async function() {
    ...
    const mergedNotes = _onlineReportNotes.map((note, noteIdx) => {
        ...
        const origItems = ((note.net_total == 0) ? [] : (note.items || [])).map((item, idx) => {
    ...
    try {
        ...
        if (res.ok && json.success) {
            ...
            window.location.href = fallbackPrintUrl;
        } else {
            if (loadingOverlay) loadingOverlay.classList.add('hidden');
            ...
            if (so.items && so.items.length > 0) {
                ...
                tbody.innerHTML = so.items.map((item, idx) => {
                    ...
                    <tr class="${blinkClass}" data-product-id="${item.product_id || 0}" data-item-id="${item.id || 0}" data-old-qty="${item.old_qty_ledger || 0}" data-has-ledger="${item.has_ledger_entry ? '1' : '0'}" data-deleted-from-sales-note="${deleteFlag}">
                        ...
                        <td class="p-2 px-3"><input type="number" value="${item.quantity}" min="1" class="e-item-qty text-xs w-16 px-2 py-1 border border-slate-200 rounded-lg focus:ring-2 focus:ring-maroon/20 outline-none text-center font-bold" data-idx="${idx}" readonly></td>
                ...
                tbody.querySelectorAll('.e-item-actual-qty, .e-item-price, .e-item-disc, .e-item-checkbox').forEach(el => {
                    el.addEventListener('input', function() {
...
/* ADDITIVE FEATURE: Read-only Force Closed Sales Notes notification history in Step 3. */
(function () {
    function escapeSalesForceClosedText(value) {
        return String(value ?? '')
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
    ...
    function formatForceClosedNumber(value) {
        const number = Number(value || 0);
        return Number.isFinite(number) ? number.toLocaleString() : '0';
    }

    function formatForceClosedDate(value) {
        const raw = String(value || '').trim();
        if (!raw) return '---';

        const parsed = new Date(raw.replace(' ', 'T'));
        if (Number.isNaN(parsed.getTime())) return escapeSalesForceClosedText(raw);
    ...
    function syncSalesForceClosedNotesButton() {
        const button = document.getElementById('sales-force-closed-notes-btn');
        if (!button) return;

        const stepThree = document.getElementById('proceed-step-3');
        const proceedModal = document.getElementById('proceed-order-modal');
        const isStepThree = !!stepThree && !stepThree.classList.contains('hidden');
        const modalOpen = !!proceedModal && !proceedModal.classList.contains('hidden');

        button.classList.toggle('hidden', !(isStepThree && modalOpen));
    }

    function renderSalesForceClosedNotes(items) {
        const list = document.getElementById('sales-force-closed-notes-list');
        const count = document.getElementById('sales-force-closed-notes-count');
        if (!list) return;

        const records = Array.isArray(items) ? items : [];
        if (count) count.textContent = records.length.toLocaleString();

        if (!records.length) {
            list.innerHTML = `
                <div class="min-h-[220px] flex flex-col items-center justify-center text-center px-6">
                    <div class="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center mb-4">
                        <i data-lucide="bell-off" class="w-7 h-7 text-slate-300"></i>
                    </div>
                    <p class="text-sm font-extrabold text-slate-600 uppercase tracking-wide">No Force Closed Notes</p>
                    <p class="text-[11px] text-slate-400 mt-2 max-w-sm">There are no Sales Note force-close notifications recorded in the audit trail.</p>
                </div>`;
            if (typeof lucide !== 'undefined') lucide.createIcons();
            return;
        }
        ...
        list.innerHTML = records.map((item) => {
            const noteNumber = escapeSalesForceClosedText(item.sales_number || '---');
            const customerName = escapeSalesForceClosedText(item.customer_name || '---');
            const closedBy = escapeSalesForceClosedText(item.closed_by || 'Unknown user');
            const closedAt = formatForceClosedDate(item.closed_at);
            const removedQty = formatForceClosedNumber(item.removed_quantity);
            const preservedQty = formatForceClosedNumber(item.preserved_invoiced_quantity);
            const invoiceCount = formatForceClosedNumber(item.invoice_count);
            const removedRows = formatForceClosedNumber((Number(item.deleted_item_count || 0) + Number(item.trimmed_item_count || 0)));

            return `
                <article class="rounded-2xl border border-red-100 bg-white shadow-sm overflow-hidden">
                    <div class="p-4 flex items-start gap-4">
                        <div class="w-11 h-11 rounded-2xl bg-red-50 flex items-center justify-center flex-shrink-0 border border-red-100">
                            <i data-lucide="archive-x" class="w-5 h-5 text-red-600"></i>
                        </div>
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                                <div class="min-w-0">
                                    <p class="text-[9px] font-bold uppercase tracking-widest text-red-500">Sales Note Force Closed</p>
                                    <h4 class="text-sm font-extrabold text-maroon mt-1 break-words">${noteNumber}</h4>
                                    <p class="text-xs text-slate-600 font-semibold mt-1 break-words">${customerName}</p>
                                </div>
                                <span class="inline-flex self-start px-2.5 py-1 bg-red-50 text-red-600 border border-red-100 rounded-full text-[9px] font-extrabold uppercase tracking-widest">Closed</span>
                            </div>

                            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 mt-4">
                                <div class="rounded-xl bg-slate-50 border border-slate-100 p-3">
                                    <p class="text-[8px] font-bold uppercase tracking-widest text-slate-400">Removed QTY</p>
                                    <p class="text-xs font-extrabold text-red-600 mt-1">${removedQty}</p>
                                </div>
                                <div class="rounded-xl bg-slate-50 border border-slate-100 p-3">
                                    <p class="text-[8px] font-bold uppercase tracking-widest text-slate-400">Affected Rows</p>
                                    <p class="text-xs font-extrabold text-slate-700 mt-1">${removedRows}</p>
                                </div>
                                <div class="rounded-xl bg-slate-50 border border-slate-100 p-3">
                                    <p class="text-[8px] font-bold uppercase tracking-widest text-slate-400">Preserved Invoiced QTY</p>
                                    <p class="text-xs font-extrabold text-emerald-600 mt-1">${preservedQty}</p>
                                </div>
                                <div class="rounded-xl bg-slate-50 border border-slate-100 p-3">
                                    <p class="text-[8px] font-bold uppercase tracking-widest text-slate-400">Invoices Preserved</p>
                                    <p class="text-xs font-extrabold text-emerald-600 mt-1">${invoiceCount}</p>
                                </div>
                            </div>

                            <div class="mt-3 pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 text-[10px] text-slate-400">
                                <span>Closed by <strong class="text-slate-600">${closedBy}</strong></span>
                                <span>${closedAt}</span>
                            </div>
                        </div>
                    </div>
    ...
    window.loadSalesForceClosedNotes = async function () {
        const list = document.getElementById('sales-force-closed-notes-list');
        const refreshButton = document.getElementById('sales-force-closed-notes-refresh-btn');
        const route = window.salesOrderRoutes?.forceClosedNotesUrl || '/admin/sales/sales-order/force-closed-notes';

        if (list) {
            list.innerHTML = `
                <div class="min-h-[220px] flex flex-col items-center justify-center text-slate-300">
                    <i data-lucide="loader-circle" class="w-8 h-8 animate-spin mb-3"></i>
                    <p class="text-xs font-bold uppercase tracking-widest">Loading notifications...</p>
                </div>`;
        }
        ...
        try {
            const response = await fetch(route + (route.includes('?') ? '&' : '?') + 'limit=50', {
                headers: {
                    'Accept': 'application/json',
                },
            });
            ...
            if (!response.ok || !result.success) {
                throw new Error(result.message || 'Unable to load Force Closed Notes.');
            }

            renderSalesForceClosedNotes(result.items || []);
        } catch (error) {
            console.error('Force Closed Sales Notes history failed:', error);
            if (list) {
                list.innerHTML = `
                    <div class="min-h-[220px] flex flex-col items-center justify-center text-center px-6">
                        <div class="w-14 h-14 rounded-2xl bg-red-50 flex items-center justify-center mb-4 border border-red-100">
                            <i data-lucide="triangle-alert" class="w-7 h-7 text-red-500"></i>
                        </div>
                        <p class="text-sm font-extrabold text-red-600 uppercase tracking-wide">Unable to load notifications</p>
                        <p class="text-[11px] text-slate-400 mt-2 max-w-md">${escapeSalesForceClosedText(error.message || 'Please try again.')}</p>
                    </div>`;
            }
            if (typeof lucide !== 'undefined') lucide.createIcons();
        } finally {
    ...
    window.openSalesForceClosedNotesModal = function () {
        const stepThree = document.getElementById('proceed-step-3');
        if (!stepThree || stepThree.classList.contains('hidden')) {
            alert('Force Closed Notes is available in Step 3 only.');
            return;
        }

        toggleModal('force-closed-sales-notes-modal', true);
        window.loadSalesForceClosedNotes();
        if (typeof lucide !== 'undefined') lucide.createIcons();
    };

    window.closeSalesForceClosedNotesModal = function () {
        toggleModal('force-closed-sales-notes-modal', false);
    };

    window.syncSalesForceClosedNotesButton = syncSalesForceClosedNotesButton;

    document.addEventListener('DOMContentLoaded', function () {
        const stepThree = document.getElementById('proceed-step-3');
        const proceedModal = document.getElementById('proceed-order-modal');
        const observer = new MutationObserver(syncSalesForceClosedNotesButton);
        if (stepThree) observer.observe(stepThree, { attributes: true, attributeFilter: ['class'] });
        if (proceedModal) observer.observe(proceedModal, { attributes: true, attributeFilter: ['class'] });
        syncSalesForceClosedNotesButton();
    });
})();
