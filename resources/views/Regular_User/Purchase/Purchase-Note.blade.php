...
@push('styles')
    <link rel="stylesheet" href="{{ asset('css/Purchase-Note.css') }}?v={{ time() }}">
@endpush

@section('purchase_note_content')
<script>
    // Debug: Log the base URL to verify it's correct (v2)
    console.log('[DEBUG v2] App URL:', '{{ config("app.url") }}');
    console.log('[DEBUG v2] Asset URL:', '{{ asset("") }}');
    console.log('[DEBUG v2] Check Password Route:', '{{ url("/check-password") }}');
    
    window.purchaseRoutes = {
        fetch: '{{ route("regular.purchase.note.data") }}',
        details: '{{ route("regular.purchase.note.details", ["id" => ":id"]) }}',
        searchSuppliers: '{{ route("regular.purchase.search-suppliers") }}',
        searchProducts: '{{ route("regular.purchase.search-products") }}',
        store: '{{ route("regular.purchase.note.store") }}',
        update: '{{ route("regular.purchase.note.update", ["id" => ":id"]) }}',
        delete: '{{ route("regular.purchase.note.delete", ["id" => ":id"]) }}',
        supplierItemConflicts: '{{ route("regular.purchase.note.supplier-item-conflicts") }}',
        forgottenPartialItems: '{{ route("regular.purchase.note.forgotten-partial-items", ["supplierCode" => ":supplierCode"]) }}',
        nextNumber: '{{ route("regular.purchase.note.next-number") }}',
        print: '{{ route("regular.purchase.note.print", ["id" => ":id"]) }}',
        verifyPasswordUrl: '{{ route("regular.purchase.note.verify-password") }}',
        checkPasswordUrl: '{{ url("/check-password") }}',
        invoiceDetailUrl: '{{ route("regular.purchase-order.detail", ["poId" => ":poId"]) }}',
        purchaseOrderIndex: '{{ route("regular.purchase-order") }}',
        invoicesByNoteUrl: '{{ route("regular.purchase-order.by-note", ["purchaseNoteNumber" => ":noteNumber"]) }}'
    };
    ...
    @if(!empty($viberData))
        window.viberPrepareNote = @json($viberData);
    @else
...
<div id="page-purchase-note-root" class="space-y-8 animate-fade-in text-slate-800">
    ...
    @include('partials.purchase.purchase_note_wizard')
...
@push('scripts')
    <script src="{{ asset('js/Purchase-Note.js') }}?bust={{ microtime(true) }}"></script>
@endpush
@endsection
