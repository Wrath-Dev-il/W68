...
@section('customer_master_content')
<script>
                    ...
                    <div id="wizard-step-1" class="space-y-6 animate-fade-in">
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="terms" id="wizard-terms" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-maroon focus:ring-4 focus:ring-maroon/5 transition-all" placeholder="e.g. Net 30 Days">
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        window.customerRoutes = {
            data: '{{ route("regular.customer-master.data") }}',
            create: '{{ route("regular.customer-master.create") }}',
            update: '{{ route("regular.customer-master.update", ["id" => ":id"]) }}',
            delete: '{{ route("regular.customer-master.delete", ["id" => ":id"]) }}',
            purchaseHistory: '{{ route("regular.customer-master.purchase-history", ["id" => ":id"]) }}',
            paymentHistory: '{{ route("regular.customer-master.payment-history", ["id" => ":id"]) }}',
            paymentHistoryDetail: '{{ route("regular.customer-master.payment-history-detail", ["id" => ":id", "salesOrderId" => ":salesOrderId"]) }}',
            notes: '{{ url("/regular/master-list/customer-master/notes/:id") }}',
            portalStatus: '{{ route("regular.customer-master.portal.status", ["id" => ":id"]) }}',
            portalGenerate: '{{ route("regular.customer-master.portal.generate", ["id" => ":id"]) }}',
            portalValidity: '{{ route("regular.customer-master.portal.validity", ["id" => ":id"]) }}',
            portalDelete: '{{ route("regular.customer-master.portal.delete", ["id" => ":id"]) }}',
            brandDiscounts: '{{ route("regular.customer-master.brand-discounts.index", ["id" => ":id"]) }}',
            brandDiscountBrands: '{{ route("regular.customer-master.brand-discounts.brands", ["id" => ":id"]) }}',
