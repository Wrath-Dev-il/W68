...
<script>
    window.prodRoutes = {
       ...
       'ledger': '{{ route("regular.prod-master.ledger", ["id" => ":id"]) }}',
        'select': '{{ route("regular.prod-master.select") }}',
        'reorderLevel': '{{ route("regular.prod-master.reorder-level") }}',
        'catalog': '{{ route("regular.prod-master.catalog") }}',
        'priceList': '{{ route("regular.prod-master.price-list") }}',
        'selectedFilters': '{{ route("regular.prod-master.selected-filters") }}',
        'activeNotes': '{{ route("regular.prod-master.active-notes", ["id" => ":id"]) }}',
        'priceCodesUpdate': '{{ route("regular.prod-master.price-codes.update", ["id" => ":id"]) }}'
    };
...
<th class="p-1.5 px-4"><input data-col="productCode2" id="search-product-code2" name="search[productCode2]" type="text" placeholder="Code 2..." class="column-search-input py-1 text-[10px] w-full"></th>
                        ...
                        <div id="view-product-edit-form" class="grid grid-cols-1 gap-y-3">
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <textarea
    id="view-prod-code"
    data-view-field="product_code"
    rows="2"
    wrap="soft"
    readonly
    class="view-product-input w-full px-2.5 py-2 rounded-lg border border-transparent bg-transparent text-xs font-bold text-slate-800 outline-none resize-none leading-5"
