...
@push('scripts')
    <script>
        ...
        window.salesRoutes = {
            customersUrl: '{{ route("regular.sales-note.customers") }}',
            productsUrl: '{{ route("regular.sales-note.products") }}',
            processUrl: '{{ route("regular.sales-note.process") }}',
            stockCheckUrl: '{{ route("regular.sales-note.check-stock") }}',
            dashboardUrl: '{{ route("regular.sales-note.dashboard") }}',
            activeUrl: '{{ route("regular.sales-note.active") }}',
            historyUrl: '{{ route("regular.sales-note.history") }}',
            detailUrl: '{{ route("regular.sales-note.detail", ["id" => ":id"]) }}',
            deleteUrl: '{{ route("regular.sales-note.delete", ["id" => ":id"]) }}',
            updateUrl: '{{ route("regular.sales-note.update", ["id" => ":id"]) }}',
            reportUrl: '{{ route("regular.sales-note.report") }}',
            checkPasswordUrl: '/check-password',
            onlinePricesUrl: '{{ route("regular.sales-note.online-prices") }}',
            printUrl: '{{ route("regular.sales-note.print") }}',
            salesOrderUrl: '{{ route("regular.sales-order") }}',
            customerPartialsUrl: '{{ route("regular.sales-note.customer.partials", ["customerId" => ":customerId"]) }}',
            remainingItemsUrl: '{{ route("regular.sales-note.remaining-items", ["noteId" => ":noteId"]) }}'
        };
    ...
    <script>
        ...
        window.loadInvoicesForEdit = async function() {
            ...
            try {
                const url = `{{ url('/regular/sales/sales-note') }}/${noteId}/invoices`;
        ...
        window.selectInvoiceForEdit = async function(invoiceId, invoiceNumber) {
            ...
            try {
                const url = `{{ url('/regular/sales/sales-note/invoice') }}/${invoiceId}/items`;
                const res = await fetch(url);
                const result = await res.json();
        ...
        window.proceedEditRemainingItems = async function() {
            ...
            try {
                const url = `{{ url('/regular/sales/sales-note') }}/${window.activeEditNoteId}/invoices`;
                const res = await fetch(url);
                const result = await res.json();
