<?php $__env->startPush('styles'); ?>
    <?php echo app('Illuminate\Foundation\Vite')(['resources/css/Waybill.css']); ?>
<?php $__env->stopPush(); ?>

<?php $__env->startSection('waybill_content'); ?>
<div id="page-waybill-root" class="space-y-8 animate-fade-in text-slate-800">
    ...
    <div id="view-history" class="main-tab-view hidden">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-slate-800">
            <div class="p-5 border-b border-slate-50 bg-slate-50/30 space-y-3">
                ...
                <div class="relative">
                    ...
                    <button id="wb-history-search-clear" onclick="clearHistorySearch()" class="absolute right-3 top-1/2 -translate-y-1/2 hidden text-slate-400 hover:text-slate-600 transition-all"><i data-lucide="x" class="w-4 h-4"></i></button>
                </div>
            </div>
            <div class="overflow-x-auto custom-scrollbar">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50/80 border-b border-slate-100">
                        ...
                        <tr class="bg-slate-50/30 border-b border-slate-100">
                            <th class="p-2 px-5"></th>
                            <th class="p-2 px-5"><input type="text" data-col="waybill_sequence" onkeyup="searchWaybillHistory(1, this.value)" placeholder="Search Sequence..." class="column-search-input"></th>
...
<div id="wb-waybill-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="w-full md:w-[70%] space-y-4">
                <div class="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm space-y-5">
                    ...
                    <div class="relative">
                        ...
                        <div id="forwarder-suggestions" class="absolute z-[980] left-0 right-0 mt-1 bg-white border border-slate-100 rounded-xl shadow-xl hidden overflow-hidden max-h-48 overflow-y-auto">
                        </div>
                    </div>
                </div>
                <div class="mt-5">
...
<div id="wb-so-select-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center">
...
<!-- ============== CONFIRM MODAL ============== -->
<div id="wb-confirm-modal" class="fixed inset-0 z-[1000] hidden flex items-center justify-center">
...
<!-- ============== SUCCESS MODAL ============== -->
<div id="wb-success-modal" class="fixed inset-0 z-[1000] hidden flex items-center justify-center">
...
<div id="wb-edit-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="w-full md:w-[30%] space-y-4">
                <div class="p-6 bg-amber-600 rounded-3xl shadow-xl text-white space-y-6 relative overflow-hidden">
                    ...
                    <div>
                        ...
                        <input type="text" id="edit-waybill-no-input" name="waybill_no" placeholder="Enter external tracking no..." class="w-full px-3 py-2 rounded-lg text-xs font-mono font-bold text-amber-800 bg-white border border-white/20 outline-none focus:ring-2 focus:ring-amber-300/50">
                    </div>
                    <div>
                        <p class="text-[9px] uppercase text-white/50 font-bold mb-1">Date</p>
...
<div id="edit-so-select-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all max-w-5xl w-full modal-animate-in mx-4 flex flex-col max-h-[90vh]">
        ...
        <div class="px-6 pt-4 pb-2">
            <div class="relative">
                ...
                <i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400"></i>
                <button id="edit-so-search-clear" onclick="clearEditSOSearch()" class="absolute right-3 top-1/2 -translate-y-1/2 hidden text-slate-400 hover:text-slate-600 transition-all"><i data-lucide="x" class="w-4 h-4"></i></button>
            </div>
        </div>
        <div class="px-6 pb-2 overflow-y-auto flex-1">
            ...
            <div id="edit-so-no-results" class="hidden p-8 text-center text-slate-300 italic">No records match your search.</div>
        </div>
...
<!-- ============== EDIT CONFIRM MODAL ============== -->
<div id="wb-edit-confirm-modal" class="fixed inset-0 z-[1000] hidden flex items-center justify-center">
...
<!-- ============== EDIT SUCCESS MODAL ============== -->
<div id="wb-edit-success-modal" class="fixed inset-0 z-[1000] hidden flex items-center justify-center">
...
<div id="wb-delete-success-modal" class="fixed inset-0 z-[1000] hidden flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:max-w-sm sm:w-full modal-animate-in mx-4">
        <div class="p-8 text-center">
            ...
            <h3 class="text-xl font-bold text-slate-800 mb-2">Success!</h3>
            <p class="text-xs text-slate-500 leading-relaxed">Waybill deleted successfully.</p>
        </div>
        <div class="p-6 pt-0">
            <button onclick="closeDeleteSuccess()" class="w-full px-4 py-3 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded-xl transition-all uppercase tracking-widest shadow-lg">OK</button>
        </div>
    </div>
</div>
...
<div id="wb-history-view-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<div id="wb-invoice-products-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center">
    ...
    <div class="flex justify-between items-start mb-8">
        <div class="space-y-1 w-1/2">
            ...
            <div class="flex items-start">
                ...
                <div class="flex-1 border-b border-slate-300 min-h-[1.25rem]">
                    <span id="print-customer-address" contenteditable="true" spellcheck="false" class="block whitespace-normal break-words leading-5"></span>
                </div>
            </div>
            <div class="flex">
        ...
        <div class="space-y-1 w-1/2 ml-8">
            ...
            <div class="flex">
                ...
                <span id="print-waybill-no" contenteditable="true" spellcheck="false" class="border-b border-slate-300 flex-1 min-w-0 min-h-5 h-auto whitespace-normal break-words leading-5 font-bold"></span>
            </div>
            <div class="flex">
...
<?php $__env->startPush('scripts'); ?>
    <script>
        window.waybillRoutes = {
            stats: "<?php echo e(route('admin.waybill.stats')); ?>",
            pending: "<?php echo e(route('admin.waybill.pending')); ?>",
            soItems: "<?php echo e(route('admin.waybill.so-items', ':id')); ?>",
            finalize: "<?php echo e(route('admin.waybill.finalize')); ?>",
            forwarderSuggestions: "<?php echo e(route('admin.waybill.forwarder-suggestions')); ?>",
            history: "<?php echo e(route('admin.waybill.history')); ?>",
            historyDetails: "<?php echo e(route('admin.waybill.history-details', ':id')); ?>",
            batchPrint: "<?php echo e(route('admin.waybill.batch-print')); ?>",
            update: "<?php echo e(route('admin.waybill.update', ':id')); ?>",
            editData: "<?php echo e(route('admin.waybill.edit-data', ':id')); ?>",
            destroy: "<?php echo e(url('admin/sales/waybill')); ?>/" + ':id'
        };
    ...
    <script src="<?php echo e(asset('js/Waybill.js')); ?>?v=<?php echo e(time()); ?>"></script>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('partials.admin.admin_sidebar_navbar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<?php /**PATH C:\xampp\htdocs\hatdog\resources\views\Admin\sales\Waybill.blade.php ENDPATH**/ ?>