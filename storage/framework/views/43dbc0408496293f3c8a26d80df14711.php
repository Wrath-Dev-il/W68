<?php $__env->startPush('styles'); ?>
    <link rel="stylesheet" href="<?php echo e(asset('css/Sales-Order.css')); ?>?v=<?php echo e(time()); ?>">
<?php $__env->stopPush(); ?>

<?php $__env->startSection('sales_order_content'); ?>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-slate-800">
        <div class="p-5 border-b border-slate-50 bg-slate-50/30">
            <div class="flex items-center space-x-4 flex-wrap">
                <button onclick="switchSalesOrderTab('active')" id="tab-active" class="sales-order-tab active">Active Notes</button>
                <button onclick="switchSalesOrderTab('history')" id="tab-history" class="sales-order-tab">Sale's Order History</button>
                <button onclick="switchSalesOrderTab('online-invoices')" id="tab-online-invoices" class="sales-order-tab">Online Invoices</button>
                <button onclick="switchSalesOrderTab('all')" id="tab-all" class="sales-order-tab">All Orders</button>
            </div>
        </div>
    </div>
...
<div id="view-sales-order-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<div id="proceed-order-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="w-full md:w-[70%] space-y-6">
                <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                    <div class="p-4 bg-slate-50 border-b border-slate-100">
                        <h4 class="text-xs font-bold text-slate-800 uppercase tracking-widest">Invoice Number</h4>
                    </div>
                    <div class="p-6">
                        <div>
                            <p class="text-[9px] text-slate-400 font-bold mb-2 ml-1 uppercase">Invoice No. <span class="text-red-500">*</span></p>
                            <input 
                                type="text" 
                                id="proceed-invoice-number" 
                                class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-maroon/20 outline-none transition-all" 
                                ...
                                <div class="mt-4">
                                    ...
                                    <p class="text-[9px] text-slate-400 mt-2 ml-1">This is the date saved on the Sales Order and used by both Sales Order and Sales Invoice print.</p>
                                </div>
                            <p class="text-[9px] text-slate-400 mt-2 ml-1">Each invoice will create a separate sales order record.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="proceed-step-2" class="proceed-step-content hidden flex flex-col gap-6 p-8 overflow-y-auto flex-1 min-h-0">
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col min-h-0">
                <div class="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center flex-shrink-0">
                    ...
                    <div id="proceed-items-scroll" class="custom-scrollbar flex-1 min-h-0">
                        <table class="w-full text-left border-collapse">
...
<div id="print-receipt-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<!-- CONFIRM ORDER MODAL -->
<div id="confirm-order-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<!-- SUCCESS MODAL -->
<div id="success-order-modal" class="fixed inset-0 z-[1000] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<!-- VIEW ORDER MODAL -->
<div id="view-order-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<!-- SELECT INVOICE FOR EDIT MODAL -->
<div id="select-invoice-edit-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
    <div onclick="toggleModal('select-invoice-edit-modal', false)" class="fixed inset-0 bg-slate-900/60 transition-opacity backdrop-blur-sm"></div>
    <div class="relative bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all max-w-3xl w-full modal-animate-in mx-4">
        <div class="bg-maroon p-6 flex justify-between items-center text-white border-b-4 border-gold">
            <div class="flex items-center space-x-4">
                <div class="p-3 bg-white/10 rounded-2xl border-2 border-gold shadow-sm">
                ...
                <div>
                    <h3 class="text-xl font-bold tracking-tight">Select Invoice to Edit</h3>
                    <p class="text-[10px] text-white/60 uppercase tracking-widest font-bold">Choose which invoice you want to modify</p>
                </div>
            </div>
            <button onclick="toggleModal('select-invoice-edit-modal', false)" class="text-white/70 hover:text-white transition-colors">
                <i data-lucide="x" class="w-6 h-6"></i>
            </button>
        </div>
        
        <div class="p-6">
            <div id="edit-note-info" class="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div class="grid grid-cols-2 gap-4 text-xs">
                    <div>
                        <span class="text-slate-400 font-bold uppercase tracking-wider">Order Number:</span>
                        <span id="edit-note-order-no" class="ml-2 text-maroon font-bold">---</span>
                    </div>
                    <div>
                        <span class="text-slate-400 font-bold uppercase tracking-wider">Customer:</span>
                        <span id="edit-note-customer" class="ml-2 text-slate-700 font-bold">---</span>
                    </div>
                </div>
            </div>

            <div id="invoice-selection-grid" class="grid grid-cols-1 md:grid-cols-2 gap-4">
...
<div id="edit-sales-order-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
        ...
        <div class="flex flex-col md:flex-row gap-6 p-6 overflow-y-auto flex-1 min-h-0">
            ...
            <div class="w-full md:w-[72%] space-y-4">
                <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                    ...
                    <div id="edit-so-blink-legend" class="hidden px-4 py-2 bg-slate-50 border-b border-slate-100 flex items-center space-x-4 text-[10px]">
                        <span class="font-bold text-slate-500 uppercase tracking-widest">Changes:</span>
                        <span class="flex items-center space-x-1"><span class="w-3 h-3 rounded-full bg-red-500 inline-block blink-item-red" style="border-left:none;animation:blink-red 1s ease-in-out 3"></span><span class="text-slate-600">Deleted</span></span>
                        <span class="flex items-center space-x-1"><span class="w-3 h-3 rounded-full bg-yellow-500 inline-block blink-item-yellow" style="border-left:none;animation:blink-yellow 1s ease-in-out 3"></span><span class="text-slate-600">Added</span></span>
                        <span class="flex items-center space-x-1"><span class="w-3 h-3 rounded-full bg-green-500 inline-block blink-item-green" style="border-left:none;animation:blink-green 1s ease-in-out infinite"></span><span class="text-slate-600">New (No Ledger)</span></span>
                    </div>
                    ...
                    <div class="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="flex items-center space-x-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">
                                ...
                                <input type="number" id="edit-so-addl-discount" value="0" min="0" max="100" step="0.01" class="w-16 px-2 py-1 border border-amber-300 rounded-lg text-xs text-center font-bold outline-none focus:ring-2 focus:ring-amber-300 bg-white">
                            </div>
...
<div id="confirm-edit-order-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<div id="success-edit-order-modal" class="fixed inset-0 z-[1000] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<!-- FILTER MODAL -->
<div id="filter-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<div id="online-report-modal" class="fixed inset-0 z-[900] hidden flex items-center justify-center" role="dialog" aria-modal="true">
    ...
    <div class="relative bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all max-w-6xl w-full modal-animate-in mx-4 flex flex-col max-h-[90vh]">
        ...
        <!-- Loading Overlay for Online Report Generation -->
        <div id="or-loading-overlay" class="hidden absolute inset-0 bg-white/80 backdrop-blur-sm rounded-2xl z-50 flex items-center justify-center">
            <div class="text-center">
                <div class="inline-block w-10 h-10 border-4 border-maroon border-t-transparent rounded-full animate-spin mb-3"></div>
                <p class="text-sm font-bold text-maroon">Generating Online Report...</p>
                <p class="text-xs text-slate-400 mt-1">Processing sales orders and updating product ledger</p>
            </div>
        </div>
    </div>
</div>
...
<div id="confirm-delete-oi-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<div id="confirm-delete-so-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<div id="counter-part-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<div id="or-item-list-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<?php $__env->startPush('scripts'); ?>
    <script>
        window.salesOrderRoutes = {
            activeUrl: '<?php echo e(route("admin.sales-order.active")); ?>',
            proceedUrl: '<?php echo e(route("admin.sales-order.proceed")); ?>',
            forceCloseNoteUrl: '<?php echo e(route("admin.sales-order.force-close-note", ["salesNoteId" => ":salesNoteId"])); ?>',
            historyUrl: '<?php echo e(route("admin.sales-order.history")); ?>',
            dashboardUrl: '<?php echo e(route("admin.sales-order.dashboard")); ?>',
            detailUrl: '<?php echo e(route("admin.sales-order.detail", ["id" => ":id"])); ?>',
            editDetailUrl: '<?php echo e(route("admin.sales-order.edit-detail", ["salesOrderId" => ":salesOrderId"])); ?>',
            updateUrl: '<?php echo e(route("admin.sales-order.update", ["salesOrderId" => ":salesOrderId"])); ?>',
            ledgerUrl: '<?php echo e(route("admin.sales-order.ledger", ["salesNumber" => ":salesNumber"])); ?>',
            receiptPrintUrl: '<?php echo e(route("admin.sales-order.receipt-print")); ?>',
            reportUrl: '<?php echo e(route("admin.sales-note.report")); ?>',
            onlinePricesUrl: '<?php echo e(route("admin.sales-note.online-prices")); ?>',
            productsUrl: '<?php echo e(route("admin.sales-note.products")); ?>',
            counterPartProductsUrl: '<?php echo e(route("admin.sales-order.counter-part.products")); ?>',
            onlineGenerateUrl: '<?php echo e(route("admin.sales-order.online-generate")); ?>',
            onlinePrintUrl: '<?php echo e(route("admin.sales-order.online-print", ["id" => ":id"])); ?>',
            onlineInvoicesUrl: '<?php echo e(route("admin.sales-order.online-invoices")); ?>',
            onlineInvoicesDataUrl: '<?php echo e(route("admin.sales-order.online-invoices-data")); ?>',
            onlineInvoiceEditUrl: '<?php echo e(route("admin.sales-order.online-edit", ["id" => ":id"])); ?>',
            onlineInvoiceUpdateUrl: '<?php echo e(route("admin.sales-order.online-update", ["id" => ":id"])); ?>',
            onlineInvoiceDeleteUrl: '<?php echo e(route("admin.sales-order.online-destroy", ["id" => ":id"])); ?>',
            onlineInvoiceMinSearchLength: 3,
            allUrl: '<?php echo e(route("admin.sales-order.all")); ?>',
            allOrdersDataUrl: '<?php echo e(route("admin.sales-order.all-orders-data")); ?>',
            historyDataUrl: '<?php echo e(route("admin.sales-order.history-data")); ?>',
            productPriceUrl: '<?php echo e(route("admin.sales-order.product-price", ["id" => ":id"])); ?>',
            priceCodesUrl: '<?php echo e(route("admin.sales-order.price-codes", ["productId" => ":productId"])); ?>',
            stockCheckUrl: '<?php echo e(route("admin.sales-order.check-stock")); ?>',
            editStockCheckUrl: '<?php echo e(route("admin.sales-order.edit-check-stock")); ?>',
            noteInvoicesUrl: '<?php echo e(route("admin.sales-note.invoices", ["noteId" => ":noteId"])); ?>',
            onlineVerifyLedgerUrl: '<?php echo e(route("admin.sales-order.online-verify-product-ledger", ["reportId" => ":reportId"])); ?>',
            historyDeleteUrl: '<?php echo e(route("admin.sales-order.history-destroy", ["id" => ":id"])); ?>'
        };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const editNoteId = urlParams.get('editNoteId');
            const salesOrderId = urlParams.get('salesOrderId');
            
            if (editNoteId && salesOrderId) {
                // Direct edit - invoice already selected in sales-note
                setTimeout(() => {
                    if (typeof switchSalesOrderTab === 'function') switchSalesOrderTab('history');
                    setTimeout(() => editSalesOrder(parseInt(salesOrderId)), 400);
                }, 600);
            } else if (editNoteId) {
                // Show invoice selection first
                setTimeout(() => {
                    if (typeof switchSalesOrderTab === 'function') switchSalesOrderTab('history');
                    setTimeout(() => showInvoiceSelectionForEdit(parseInt(editNoteId)), 400);
                }, 600);
            }
        });
    </script>
    <script src="<?php echo e(asset('js/Sales-Order.js')); ?>?v=<?php echo e(time()); ?>"></script>
    <script>
        let currentStep = 1;
...
<?php /**PATH C:\xampp\htdocs\hatdog\resources\views/Admin/sales/sales-order.blade.php ENDPATH**/ ?>