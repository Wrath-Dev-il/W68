...
<?php $__env->startPush('scripts'); ?>
    <script>
        ...
        window.salesRoutes = {
            customersUrl: '<?php echo e(route("regular.sales-note.customers")); ?>',
            productsUrl: '<?php echo e(route("regular.sales-note.products")); ?>',
            processUrl: '<?php echo e(route("regular.sales-note.process")); ?>',
            stockCheckUrl: '<?php echo e(route("regular.sales-note.check-stock")); ?>',
            dashboardUrl: '<?php echo e(route("regular.sales-note.dashboard")); ?>',
            activeUrl: '<?php echo e(route("regular.sales-note.active")); ?>',
            historyUrl: '<?php echo e(route("regular.sales-note.history")); ?>',
            detailUrl: '<?php echo e(route("regular.sales-note.detail", ["id" => ":id"])); ?>',
            deleteUrl: '<?php echo e(route("regular.sales-note.delete", ["id" => ":id"])); ?>',
            updateUrl: '<?php echo e(route("regular.sales-note.update", ["id" => ":id"])); ?>',
            reportUrl: '<?php echo e(route("regular.sales-note.report")); ?>',
            checkPasswordUrl: '/check-password',
            onlinePricesUrl: '<?php echo e(route("regular.sales-note.online-prices")); ?>',
            printUrl: '<?php echo e(route("regular.sales-note.print")); ?>',
            salesOrderUrl: '<?php echo e(route("regular.sales-order")); ?>',
            customerPartialsUrl: '<?php echo e(route("regular.sales-note.customer.partials", ["customerId" => ":customerId"])); ?>',
            remainingItemsUrl: '<?php echo e(route("regular.sales-note.remaining-items", ["noteId" => ":noteId"])); ?>'
        };
    ...
    <script>
        ...
        window.loadInvoicesForEdit = async function() {
            ...
            try {
                const url = `<?php echo e(url('/regular/sales/sales-note')); ?>/${noteId}/invoices`;
        ...
        window.selectInvoiceForEdit = async function(invoiceId, invoiceNumber) {
            ...
            // Clear tracking sets when selecting a new invoice
            window._editAddedProductCodes = new Set();
            window._editDeletedProductCodes = new Set();
            window._pendingDeletedItemIds = new Set();

            try {
                const url = `<?php echo e(url('/regular/sales/sales-note/invoice')); ?>/${invoiceId}/items`;
                const res = await fetch(url);
                const result = await res.json();
                ...
                if (result.success) {
                    ...
                    if (result.items && result.items.length > 0) {
                        result.items.forEach(item => {
                            ...
                            const remainingVal = Number(item.remaining_qty ?? Math.max(0, Number(item.quantity || 0) - Number(item.actual_qty || 0)));
                            row.setAttribute('data-remaining-qty', remainingVal);
        ...
        window.proceedEditRemainingItems = async function() {
            ...
            try {
                const url = `<?php echo e(url('/regular/sales/sales-note')); ?>/${window.activeEditNoteId}/invoices`;
                const res = await fetch(url);
                const result = await res.json();
        ...
        window.finalizeEditSalesNote = async function() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Redirect to Sales Order with proceed_note_data to auto-open Proceed modal
                    if (!window.activeEditInvoiceId) {
                        ...
                        window.location.href = window.salesRoutes.salesOrderUrl || '/regular/sales/sales-order';
                        return;
                    }
                    ...
                    window.location.href = (window.salesRoutes.salesOrderUrl || '/regular/sales/sales-order') + '?' + params.toString();
                } else {
...
<?php echo $__env->make('partials.user_account.user_sidebar_navbar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\hatdog\resources\views\Regular_User\sales\sales-note.blade.php ENDPATH**/ ?>