...
<script>
    window.adjustmentRoutes = {
        fetch: '<?php echo e(route("special.inv-adjust.data")); ?>',
        adjust: '<?php echo e(route("special.inv-adjust.save")); ?>',
        adjusted: '<?php echo e(route("special.inv-adjust.adjusted")); ?>',
        adjustedUpdate: '<?php echo e(route("special.inv-adjust.adjusted.update", ["id" => ":id"])); ?>',
        adjustedDelete: '<?php echo e(route("special.inv-adjust.adjusted.delete", ["id" => ":id"])); ?>',
        ledger: '<?php echo e(route("special.prod-master.ledger", ["id" => ":id"])); ?>'
    };
</script>
<div id="page-inventory-adjustment-root" class="space-y-8 animate-fade-in p-6">
    ...
    <div class="flex items-center gap-2 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm w-fit">
        <button id="tab-adjust-btn" onclick="switchAdjustmentTab('adjust')" class="px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest bg-maroon text-white transition-all">Adjust Inventory</button>
        <button id="tab-adjusted-btn" onclick="switchAdjustmentTab('adjusted')" class="px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-all">Adjusted</button>
    </div>

    <div id="adjust-tab-panel">
    <!-- MAIN TABLE -->
    ...
    <!-- ADJUSTED TAB PANEL -->
    <div id="adjusted-tab-panel" class="hidden bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="overflow-x-auto custom-scrollbar">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 text-slate-400 text-[10px] font-bold uppercase tracking-widest">
        ...
        <div class="p-5 border-t border-slate-100 bg-slate-50/30 flex items-center justify-between flex-shrink-0">
            <span id="adjusted-pagination-info" class="text-xs text-slate-500 font-medium">Showing 0 to 0 of 0 entries</span>
            <div id="adjusted-pagination-container" class="flex items-center space-x-2"></div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="modal-animate-in relative bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden flex flex-col">
                ...
                <div class="p-8 space-y-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase ml-1">Unit</label>
                        <input type="text" id="adj-input-unit" maxlength="255" class="w-full px-5 py-3.5 bg-white border-2 border-slate-100 rounded-2xl text-sm font-black text-slate-800 outline-none focus:border-maroon focus:ring-4 focus:ring-maroon/5 transition-all" placeholder="PCS">
                    </div>
    ...
    <div id="edit-adjustment-modal" class="fixed inset-0 z-[90] overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen p-4">
            <div onclick="toggleModal('edit-adjustment-modal', false)" class="fixed inset-0 bg-maroon-900/60 backdrop-blur-sm"></div>
            <div class="modal-animate-in relative bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
                <div class="bg-maroon-gradient px-8 py-5 flex items-center justify-between">
                    <div class="flex items-center space-x-3 text-white">
                        <i data-lucide="pencil" class="w-5 h-5 text-gold"></i>
                        <h3 class="text-sm font-bold uppercase tracking-widest">Edit Adjustment</h3>
                    </div>
                    <button onclick="toggleModal('edit-adjustment-modal', false)" class="text-white/70 hover:text-white p-2 hover:bg-white/10 rounded-full transition-all"><i data-lucide="x" class="w-6 h-6"></i></button>
                </div>
                <div class="p-8 space-y-5">
                    <input type="hidden" id="edit-adjustment-id">
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Product Code</label>
                        <input id="edit-adjustment-code" type="text" readonly class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-sm font-black text-slate-700 cursor-not-allowed">
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Part No</label>
                        <input id="edit-adjustment-part" type="text" readonly class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-sm font-bold text-slate-600 cursor-not-allowed">
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Qty</label>
                        <input id="edit-adjustment-qty" type="number" min="0" class="w-full px-4 py-3 bg-white border-2 border-slate-100 rounded-xl text-lg font-black text-slate-800 outline-none focus:border-maroon" placeholder="0">
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Unit</label>
                        <input id="edit-adjustment-unit" type="text" maxlength="255" class="w-full px-4 py-3 bg-white border-2 border-slate-100 rounded-xl text-sm font-black text-slate-800 outline-none focus:border-maroon" placeholder="PCS">
                    </div>
                    <div class="grid grid-cols-2 gap-3 pt-3">
                        <button onclick="toggleModal('edit-adjustment-modal', false)" class="py-3.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded-xl uppercase tracking-widest">Cancel</button>
                        <button onclick="saveEditedAdjustment()" class="py-3.5 bg-maroon text-white text-[10px] font-bold rounded-xl shadow-lg uppercase tracking-widest">Save</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="delete-adjustment-modal" class="fixed inset-0 z-[100] overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="modal-animate-in relative bg-white rounded-3xl shadow-2xl max-w-sm w-full p-8 text-center space-y-6">
                <div class="w-20 h-20 rounded-full bg-red-50 mx-auto flex items-center justify-center shadow-inner"><i data-lucide="trash-2" class="w-9 h-9 text-red-500"></i></div>
                <div>
                    <h3 class="text-lg font-black text-slate-800">Delete Adjustment?</h3>
                    <p id="delete-adjustment-text" class="text-xs text-slate-400 font-medium mt-2">This will delete the selected adjustment ledger record.</p>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <button onclick="toggleModal('delete-adjustment-modal', false)" class="py-3.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded-xl uppercase tracking-widest">Cancel</button>
                    <button id="delete-adjustment-yes" onclick="confirmDeleteAdjustment()" class="py-3.5 bg-red-600 text-white text-[10px] font-bold rounded-xl uppercase tracking-widest">Delete</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="success-modal" class="fixed inset-0 z-[110] overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="modal-animate-in relative bg-white rounded-3xl shadow-2xl max-w-sm w-full p-10 text-center space-y-6 border-b-8 border-emerald-500">
                ...
                <div>
                    <h3 id="success-modal-title" class="text-xl font-black text-slate-800 tracking-tight">Adjustment Complete!</h3>
                    <p id="success-modal-message" class="text-xs text-slate-400 font-medium leading-relaxed mt-2">The inventory levels have been synchronized successfully.</p>
                </div>
...
<?php $__env->startPush('scripts'); ?>
    <script src="<?php echo e(asset('js/inventory_adjustment.js')); ?>?v=<?php echo e(filemtime(public_path('js/inventory_adjustment.js'))); ?>"></script>
<?php $__env->stopPush(); ?>




<?php echo $__env->make('partials.special_user.special_sidebar_navbar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\hatdog\resources\views/Special_User/Inventory/inventory_adjustment.blade.php ENDPATH**/ ?>