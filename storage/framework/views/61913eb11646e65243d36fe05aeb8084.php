<?php $__env->startPush('styles'); ?>
    <link rel="stylesheet" href="<?php echo e(asset('css/Purchase-Order.css')); ?>?v=<?php echo e(time()); ?>">
<?php $__env->stopPush(); ?>

<?php $__env->startSection('purchase_order_content'); ?>
<script>
    window.purchaseRoutes = {
        process: '<?php echo e(route("regular.purchase-order.process")); ?>',
        updatePO: '<?php echo e(route("regular.purchase-order.update")); ?>',
        openNotesUrl: '<?php echo e(route("regular.purchase.open-notes")); ?>',
        dashboardDataUrl: '<?php echo e(route("regular.purchase-order.dashboard")); ?>',
        activeDataUrl: '<?php echo e(route("regular.purchase-order.active")); ?>',
        historyDataUrl: '<?php echo e(route("regular.purchase-order.history")); ?>',
        poItemsUrl: '<?php echo e(route("regular.purchase-order.po-items", ["poId" => ":poId"])); ?>',
        poDetailUrl: '<?php echo e(route("regular.purchase-order.detail", ["poId" => ":poId"])); ?>',
        forceCloseUrl: '<?php echo e(route("regular.purchase-order.force-close.submit", ["purchaseNoteId" => ":purchaseNoteId"])); ?>',
    };
                        ...
                        <div class="p-6 bg-white rounded-3xl border border-slate-100 space-y-6 shadow-md relative overflow-hidden">
                            ...
                            <div class="pt-4 border-t border-slate-100 flex justify-between items-center">
                                ...
                                <div class="text-right">
                                    ...
                                    <input type="date" id="proceed-transaction-date" value="<?php echo e(now()->format('Y-m-d')); ?>" class="px-3 py-2 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 bg-white focus:ring-2 focus:ring-maroon/20 focus:border-maroon outline-none">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex-1 overflow-hidden border-2 border-slate-200 rounded-2xl bg-white shadow-inner">
                            <div id="print-preview-container" class="h-full overflow-auto custom-scrollbar p-4" style="transform-origin: top left;">
                                <div id="print-layout-content" style="width: 8.5in; transform: scale(0.7); transform-origin: top left;">
                                    ...
                                    <?php echo $__env->make('partials.purchase.charge-receiving-layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                                </div>
                            </div>
                        </div>
    ...
    <div class="relative bg-white rounded-3xl p-8 shadow-2xl transform transition-all max-w-lg w-full modal-animate-in mx-4 border-t-8 border-maroon z-[421]">
        <div class="flex items-start gap-4 mb-6">
            <div id="po-sync-spinner" class="h-14 w-14 rounded-2xl bg-maroon/10 border border-maroon/20 flex items-center justify-center shrink-0">
                <i data-lucide="loader-circle" class="w-8 h-8 text-maroon animate-spin"></i>
            </div>
            <div id="po-sync-success-icon" class="hidden h-14 w-14 rounded-2xl bg-green-100 border border-green-200 items-center justify-center shrink-0">
                <i data-lucide="check-circle-2" class="w-8 h-8 text-green-600"></i>
            </div>
            <div id="po-sync-error-icon" class="hidden h-14 w-14 rounded-2xl bg-red-100 border border-red-200 items-center justify-center shrink-0">
                <i data-lucide="circle-alert" class="w-8 h-8 text-red-600"></i>
            </div>
            <div>
                <h3 id="po-sync-title" class="text-xl font-extrabold text-slate-800 tracking-tight">Analyzing Purchase Order</h3>
                <p id="po-sync-message" class="text-sm text-slate-500 mt-1 leading-relaxed">Please wait while the transaction is being recorded.</p>
            </div>
        </div>

        <div class="space-y-2 mb-6">
            <div data-po-sync-step="purchase_orders" class="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
            ...
            <div data-po-sync-step="purchase_order_items" class="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
                <span class="text-xs font-bold text-slate-600">Purchase Order Items</span>
                <span class="po-sync-step-status text-[10px] font-bold uppercase tracking-widest text-slate-400">Pending</span>
            </div>
            <div data-po-sync-step="product_ledgers" class="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
                <span class="text-xs font-bold text-slate-600">Product Ledgers</span>
                <span class="po-sync-step-status text-[10px] font-bold uppercase tracking-widest text-slate-400">Pending</span>
            </div>
            <div data-po-sync-step="supplier_ledgers" class="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
                <span class="text-xs font-bold text-slate-600">Supplier Ledger</span>
                <span class="po-sync-step-status text-[10px] font-bold uppercase tracking-widest text-slate-400">Pending</span>
            </div>
            <div data-po-sync-step="product_costs" class="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
                <span class="text-xs font-bold text-slate-600">Product Costs</span>
                <span class="po-sync-step-status text-[10px] font-bold uppercase tracking-widest text-slate-400">Pending</span>
            </div>
        </div>
...
<!-- 7. SUCCESS MODAL -->
<div id="success-modal" class="fixed inset-0 z-[500] hidden flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="success-modal-title">
    <div class="fixed inset-0 bg-slate-900/80 transition-opacity backdrop-blur-md z-[500]"></div>
    <div class="relative bg-white rounded-3xl text-center p-10 shadow-2xl transform transition-all max-w-md w-full modal-animate-in mx-4 border-t-8 border-green-500 z-[501]">
        ...
        <h3 id="success-modal-title" class="text-2xl font-extrabold text-slate-800 mb-2">Purchase Order Saved!</h3>
        <p id="success-msg" class="text-sm text-slate-500 mb-3 leading-relaxed">The Purchase Order has been successfully processed and recorded in the system.</p>
        <p class="text-xs text-slate-400 mb-8">Click Continue to refresh the page and display the latest records.</p>
        <button id="success-reload-btn" type="button" onclick="completeSuccessfulPOProcess()" class="w-full px-4 py-4 bg-maroon text-white rounded-2xl text-xs font-bold hover:bg-maroon-800 transition-all shadow-xl uppercase tracking-widest">Continue &amp; Reload</button>
    </div>
</div>
...
<?php $__env->startPush('scripts'); ?>
    <script>
        window.purchaseRoutes = {
            process: '<?php echo e(route("regular.purchase-order.process")); ?>',
            updatePO: '<?php echo e(route("regular.purchase-order.update")); ?>',
            itemsUrl: '<?php echo e(url("/regular/purchase/purchase-order/items")); ?>',
            openNotesUrl: '<?php echo e(route("regular.purchase.open-notes")); ?>',
            activeDataUrl: '<?php echo e(route("regular.purchase-order.active")); ?>',
            historyDataUrl: '<?php echo e(route("regular.purchase-order.history")); ?>',
            detailsByNumberUrl: '<?php echo e(url("/regular/purchase/purchase-note/details-by-number")); ?>',
            dashboardDataUrl: '<?php echo e(route("regular.purchase-order.dashboard")); ?>',
            poItemsUrl: '<?php echo e(route("regular.purchase-order.po-items", ["poId" => ":poId"])); ?>',
            poDetailUrl: '<?php echo e(route("regular.purchase-order.detail", ["poId" => ":poId"])); ?>',
            forceCloseUrl: '<?php echo e(route("regular.purchase-order.force-close.submit", ["purchaseNoteId" => ":purchaseNoteId"])); ?>',
    ...
    <script src="<?php echo e(asset('js/Purchase-Order.js')); ?>?v=<?php echo e(time()); ?>"></script>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('partials.user_account.user_sidebar_navbar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\hatdog\resources\views/Regular_User/Purchase/Purchase-Order.blade.php ENDPATH**/ ?>