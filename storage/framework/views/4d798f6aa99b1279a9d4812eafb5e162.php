...
<script>
    // Debug: Log the base URL to verify it's correct (v2)
    console.log('[DEBUG v2] App URL:', '<?php echo e(config("app.url")); ?>');
    console.log('[DEBUG v2] Asset URL:', '<?php echo e(asset("")); ?>');
    console.log('[DEBUG v2] Check Password Route:', '<?php echo e(url("/check-password")); ?>');
    
    window.purchaseRoutes = {
        fetch: '<?php echo e(route("regular.purchase.note.data")); ?>',
        details: '<?php echo e(route("regular.purchase.note.details", ["id" => ":id"])); ?>',
        searchSuppliers: '<?php echo e(route("regular.purchase.search-suppliers")); ?>',
        searchProducts: '<?php echo e(route("regular.purchase.search-products")); ?>',
        store: '<?php echo e(route("regular.purchase.note.store")); ?>',
        update: '<?php echo e(route("regular.purchase.note.update", ["id" => ":id"])); ?>',
        delete: '<?php echo e(route("regular.purchase.note.delete", ["id" => ":id"])); ?>',
        supplierItemConflicts: '<?php echo e(route("regular.purchase.note.supplier-item-conflicts")); ?>',
        forgottenPartialItems: '<?php echo e(route("regular.purchase.note.forgotten-partial-items", ["supplierCode" => ":supplierCode"])); ?>',
        nextNumber: '<?php echo e(route("regular.purchase.note.next-number")); ?>',
        print: '<?php echo e(route("regular.purchase.note.print", ["id" => ":id"])); ?>',
        verifyPasswordUrl: '<?php echo e(route("regular.purchase.note.verify-password")); ?>',
        checkPasswordUrl: '<?php echo e(url("/check-password")); ?>',
        invoiceDetailUrl: '<?php echo e(route("regular.purchase-order.detail", ["poId" => ":poId"])); ?>',
        purchaseOrderIndex: '<?php echo e(route("regular.purchase-order")); ?>',
        invoicesByNoteUrl: '<?php echo e(route("regular.purchase-order.by-note", ["purchaseNoteNumber" => ":noteNumber"])); ?>'
    };
    
    // Debug: Log all routes (v2)
    console.log('[DEBUG v2] Purchase Routes:', window.purchaseRoutes);

    <?php if(!empty($viberData)): ?>
...
<div id="page-purchase-note-root" class="space-y-8 animate-fade-in text-slate-800">
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/80 border-b border-slate-100">
                    ...
                    <tr class="bg-slate-50/30">
                        <th class="p-2 px-3"><input type="text" data-col="poNumber" name="purchase_note_number" placeholder="Search PO..." class="column-search-input text-[9px] w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-maroon/10"></th>
    ...
    <div id="supplier-search-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
    ...
    <div id="item-search-modal" class="fixed inset-0 z-[950] hidden flex items-center justify-center" role="dialog" aria-modal="true">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3 text-slate-800">
                        <div>
                            ...
                            <input type="number" placeholder="₱ 0.00" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-maroon/20 focus:border-maroon outline-none transition-all">
                        </div>
                        <div>
    ...
    <div id="item-exists-modal" class="fixed inset-0 z-[960] hidden flex items-center justify-center" role="dialog" aria-modal="true">
...
<?php $__env->startPush('scripts'); ?>
    <script src="<?php echo e(asset('js/Purchase-Note.js')); ?>?bust=<?php echo e(microtime(true)); ?>"></script>
<?php $__env->stopPush(); ?>
<?php $__env->stopSection(); ?>


<?php echo $__env->make('partials.user_account.user_sidebar_navbar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\hatdog\resources\views\Regular_User\Purchase\Purchase-Note.blade.php ENDPATH**/ ?>