        ...
        <div class="profile-card rounded-2xl shadow-xl p-8 text-white">
            <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                <div class="relative">
                    <div class="w-28 h-28 rounded-full border-4 border-goldlining-400 overflow-hidden bg-maroon-800 flex items-center justify-center shadow-lg">
                        <?php if($profilePictureUrl): ?>
                            <img src="<?php echo e($profilePictureUrl); ?>" alt="Profile" class="w-full h-full object-cover">
                        <?php else: ?>
                ...
                <div class="text-center md:text-left flex-1">
                    <h1 class="text-2xl font-extrabold"><?php echo e($profileUser->User_First_Name ?? ''); ?> <?php echo e($profileUser->User_Middle_Name ?? ''); ?> <?php echo e($profileUser->User_Last_Name ?? ''); ?></h1>
                    <p class="text-goldlining-400 font-semibold text-sm mt-1"><?php echo e($accountTypeLabel); ?></p>
                    <p class="text-gray-300 text-xs mt-2">User ID: <?php echo e($profileUser->User_ID ?? 'N/A'); ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            ...
            <form id="profile-form" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="User_First_Name" value="<?php echo e($profileUser->User_First_Name ?? ''); ?>" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-maroon-500 focus:border-transparent">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Middle Name</label>
                    <input type="text" name="User_Middle_Name" value="<?php echo e($profileUser->User_Middle_Name ?? ''); ?>" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-maroon-500 focus:border-transparent">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Last Name</label>
                    <input type="text" name="User_Last_Name" value="<?php echo e($profileUser->User_Last_Name ?? ''); ?>" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-maroon-500 focus:border-transparent">
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="stat-card bg-gradient-to-br from-maroon-50 to-maroon-100 rounded-xl p-5 border border-maroon-200">
                    ...
                    <p class="text-2xl font-extrabold text-maroon-900 mt-1"><?php echo e($totalLoginCount); ?></p>
                </div>
                <div class="stat-card bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-5 border border-amber-200">
                    ...
                    <p class="text-2xl font-extrabold text-amber-900 mt-1"><?php echo e($weeklyLoginCount); ?></p>
                </div>
                <div class="stat-card bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-5 border border-emerald-200">
                    ...
                    <p class="text-sm font-bold text-emerald-900 mt-1"><?php echo e($latestLoginAt); ?></p>
                </div>
            </div>
        </div>
...
document.getElementById('profile-pic-input')?.addEventListener('change', function(e) {
    ...
    fetch('<?php echo e(route("warehouse.profile.picture.update")); ?>', {
        method: 'POST',
        body: formData,
    })
    .then(r => r.json())
...
document.getElementById('profile-form')?.addEventListener('submit', function(e) {
    ...
    formData.append('_token', '<?php echo e(csrf_token()); ?>');
    fetch('<?php echo e(route("warehouse.profile.update")); ?>', {
        method: 'POST',
        body: formData,
    })
    .then(r => r.json())
...
<?php /**PATH C:\xampp\htdocs\hatdog\resources\views\partials\Warehouse_User\Warehouse-profile.blade.php ENDPATH**/ ?>