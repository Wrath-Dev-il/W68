...
<body class="bg-slate-200 font-sans">
    ...
    <div class="fixed top-5 left-5 z-[250] no-print">
        <a href="<?php echo e(route('special.prod-master')); ?>" class="px-6 py-3 bg-gold text-maroon-900 font-black rounded-2xl shadow-2xl hover:bg-goldlining-500 transition-all flex items-center gap-3 border border-goldlining-500">
        ...
        <div class="no-print bg-amber-50 border-l-4 border-amber-400 p-4 mb-6 rounded-r-xl shadow-sm mx-auto max-w-[215.9mm]">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-xs text-amber-700">Showing first 180 items for preview. Use the <b>PRINT LIST</b> button to generate the full report for all <?php echo e(number_format($totalCount)); ?> items.</p>
                </div>
            </div>
        </div>
    <?php endif; ?>

    <?php $__currentLoopData = $previewChunks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pageIdx => $pageProducts): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="report-page catalog-page-item" data-page="<?php echo e($pageIdx + 1); ?>" style="min-height: auto; height: auto;">
        ...
        <div class="report-content">
            ...
            <!-- Footer -->
            <?php $pageDescs = $pageProducts->pluck('description')->filter()->unique()->values(); ?>
            <div class="mt-auto pt-4 border-t-2 border-slate-300 flex justify-between items-center">
                <p class="text-[12px] text-black font-bold uppercase tracking-widest">
                    ...
                    <?php if($pageDescs->isNotEmpty()): ?>
                    <br><span class="text-[10px] text-slate-600 font-bold">[<?php echo e($pageDescs->implode(' | ')); ?>]</span>
                    <?php endif; ?>
                </p>
                <p class="text-[12px] text-black font-bold uppercase">PAGE <?php echo e($pageIdx + 1); ?> OF <?php echo e($previewTotalPages); ?></p>
            </div>
        </div>
    </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    <div id="print-report">
        <?php $__currentLoopData = $printChunks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pageIdx => $pageProducts): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $itemsArr = $pageProducts->values(); $lastDesc = null; ?>
        <div class="print-page-item">
            ...
            <table class="print-table">
                ...
                <tbody>
                    <?php $__currentLoopData = $itemsArr; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php $desc = trim(strtoupper($product->description ?: 'NO DESCRIPTION')); ?>
                    <?php if($desc !== $lastDesc): ?>
                    <tr>
                    ...
                    <?php endif; ?>
                    <tr>
                        <td style="border:2px solid black;width:15%;"><?php echo e($product->product_code); ?></td>
                        <td style="border:2px solid black;width:20%;font-family:monospace;"><?php echo e($product->part_number ?: '---'); ?></td>
                        <td style="border:2px solid black;width:35%;">
                            <div><?php echo e($product->application ?: $product->Application ?: '---'); ?></div>
                            <?php $pos = $product->position ?: $product->Position; ?>
                            <?php if($pos): ?>
                        ...
                        <td style="border:2px solid black;width:15%;text-transform:uppercase;"><?php echo e($product->category); ?></td>
                        <td style="border:2px solid black;width:15%;" class="price-col">₱<?php echo e(number_format($product->selling_price, 2)); ?></td>
                    </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </tbody>
            </table>
            <?php $printDescs = $itemsArr->pluck('description')->filter()->unique()->values(); ?>
            <div style="border-top:1px solid black;display:flex;justify-content:space-between;align-items:center;padding-top:2mm;margin-top:8px;">
                <span style="font-size:12px;font-weight:700;text-transform:uppercase;color:black;">W68 AUTO PARTS &mdash; PRICE LIST <?php if($printDescs->isNotEmpty()): ?><br><span style="font-size:10px;">[<?php echo e($printDescs->implode(' | ')); ?>]</span><?php endif; ?></span>
                <span style="font-size:12px;font-weight:700;text-transform:uppercase;color:black;white-space:nowrap;">PAGE <?php echo e($pageIdx + 1); ?> OF <?php echo e($printTotalPages); ?></span>
            </div>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
...
<?php /**PATH C:\xampp\htdocs\hatdog\resources\views\Special_User\master_list\Product_PriceList.blade.php ENDPATH**/ ?>