<?php $__env->startPush('styles'); ?>
    <link rel="stylesheet" href="<?php echo e(asset('css/Sales-Note.css')); ?>?v=<?php echo e(time()); ?>">
<?php $__env->stopPush(); ?>
...
<div id="page-sales-note-root" class="space-y-8 animate-fade-in text-slate-800">
    ...
    <div id="add-sales-note-modal" class="fixed inset-0 z-[2000] hidden flex items-center justify-center" role="dialog" aria-modal="true">
        <div onclick="checkAndCloseSalesNoteModal()" class="fixed inset-0 bg-slate-900/60 transition-opacity backdrop-blur-sm"></div>
        <div class="relative bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all w-[96vw] max-w-none modal-animate-in mx-2 flex flex-col max-h-[95vh]">
            ...
            <div class="bg-maroon p-6 flex justify-between items-center text-white border-b-4 border-gold">
                ...
                <button onclick="checkAndCloseSalesNoteModal()" class="text-white/70 hover:text-white transition-colors bg-white/10 p-2 rounded-xl">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-8">
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead class="bg-slate-50 border-b border-slate-100">
                                    <tr class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                        ...
                                        <th class="p-4 px-6 text-center">Disc % / Bonus</th>
                                    </tr>
                ...
                <!-- STEP 4: DETAILS REVIEW (30/70 SPLIT) -->
                            ...
                            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                                ...
                                <div class="p-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                                    ...
                                    <div class="flex space-x-3">
                                        <button onclick="goToStep(2)" class="px-8 py-3 bg-white border border-slate-200 text-slate-600 rounded-2xl text-xs font-bold hover:bg-slate-50 transition-all uppercase tracking-widest">Edit</button>
            ...
            <div class="p-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                ...
                <div class="flex space-x-3">
                    <button onclick="checkAndCloseSalesNoteModal()" class="px-6 py-2.5 text-xs font-bold text-slate-400 hover:text-slate-600 transition-all uppercase tracking-widest">Cancel</button>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-8">
                ...
                <div id="edit-sales-step-3" class="edit-sales-step-content hidden space-y-6">
                    <div class="flex items-center justify-between">
                        <h4 id="edit-step3-heading" class="text-sm font-bold text-slate-700 uppercase tracking-widest">Edit Items</h4>
                    </div>
                    <div class="flex items-center space-x-3 mb-4">
                        <div class="flex-1 relative">
                            <input type="text" onclick="loadProducts()" placeholder="Search and select items to add..." readonly class="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm cursor-pointer hover:border-maroon transition-all shadow-sm">
...
<!-- DISCARD SALES NOTE MODAL -->
<div id="discard-sales-note-modal" class="fixed inset-0 z-[2080] hidden flex items-center justify-center" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-slate-900/80 transition-opacity backdrop-blur-md"></div>
    <div class="relative bg-white rounded-3xl text-center p-8 shadow-2xl transform transition-all max-w-sm w-full modal-animate-in mx-4 border-t-8 border-amber-500">
        ...
        <div class="flex space-x-3">
            <button onclick="toggleModal('discard-sales-note-modal', false)" class="flex-1 px-4 py-3 bg-slate-100 text-slate-600 rounded-2xl text-xs font-bold hover:bg-slate-200 transition-all uppercase tracking-widest">No, Keep Editing</button>
            <button onclick="confirmDiscardSalesNoteModal()" class="flex-1 px-4 py-3 bg-amber-500 text-white rounded-2xl text-xs font-bold hover:bg-amber-600 transition-all shadow-lg uppercase tracking-widest">Yes, Discard</button>
        </div>
    </div>
</div>
...
<?php $__env->startPush('scripts'); ?>
    <script>
        ...
        window.salesRoutes = {
            customersUrl: '<?php echo e(route("special.sales-note.customers")); ?>',
            productsUrl: '<?php echo e(route("special.sales-note.products")); ?>',
            processUrl: '<?php echo e(route("special.sales-note.process")); ?>',
            stockCheckUrl: '<?php echo e(route("special.sales-note.check-stock")); ?>',
            dashboardUrl: '<?php echo e(route("special.sales-note.dashboard")); ?>',
            activeUrl: '<?php echo e(route("special.sales-note.active")); ?>',
            historyUrl: '<?php echo e(route("special.sales-note.history")); ?>',
            detailUrl: '<?php echo e(route("special.sales-note.detail", ["id" => ":id"])); ?>',
            deleteUrl: '<?php echo e(route("special.sales-note.delete", ["id" => ":id"])); ?>',
            updateUrl: '<?php echo e(route("special.sales-note.update", ["id" => ":id"])); ?>',
            reportUrl: '<?php echo e(route("special.sales-note.report")); ?>',
            checkPasswordUrl: '/check-password',
            onlinePricesUrl: '<?php echo e(route("special.sales-note.online-prices")); ?>',
            printUrl: '<?php echo e(route("special.sales-note.print")); ?>',
            salesOrderUrl: '<?php echo e(route("special.sales-order")); ?>',
            customerPartialsUrl: '<?php echo e(route("special.sales-note.customer.partials", ["customerId" => ":customerId"])); ?>',
            remainingItemsUrl: '<?php echo e(route("special.sales-note.remaining-items", ["noteId" => ":noteId"])); ?>'
        };
        window.csrfToken = '<?php echo e(csrf_token()); ?>';
        window.reportParams = {
            ids: '<?php echo e($report_ids ?? ""); ?>',
            type: '<?php echo e($report_type ?? "note"); ?>',
            outDate1: '<?php echo e($report_out_date1 ?? date("Y")); ?>',
            outDate2: '<?php echo e($report_out_date2 ?? date("Y") + 1); ?>',
            outDate3: '<?php echo e($report_out_date3 ?? ""); ?>',
            onlinePrices: '<?php echo e($online_prices_confirmed ?? false); ?>' === '1'
        };

        document.addEventListener('DOMContentLoaded', function() {
            if (typeof loadSalesDashboard === 'function') loadSalesDashboard();
            if (typeof loadSalesActive === 'function') loadSalesActive();
            <?php if(isset($online_prices_confirmed) && $online_prices_confirmed): ?>
    ...
    <script src="<?php echo e(asset('js/Sales-Note.js')); ?>?v=<?php echo e(time()); ?>"></script>
    <script>
    ...
    <script>
        ...
        window.goToStep = function(step) {
            ...
            if (step === 1) {
                ...
                nextText.innerText = step === totalSteps - 1 ? 'Review Sale' : 'Next Step';
                if(nextIcon) nextIcon.setAttribute('data-lucide', step === totalSteps - 1 ? 'eye' : 'chevron-right');
            } else {
                btnBack.classList.remove('hidden');
    ...
    <script>
        /**
         * Checks if the Add/Edit Sales Note modal has any user-entered data.
         * If yes, shows a "Discard Changes?" confirmation.
         * If no, closes the modal directly.
         */
        function checkAndCloseSalesNoteModal() {
            if (salesNoteModalHasData()) {
                toggleModal('discard-sales-note-modal', true);
            } else {
        ...
        /**
         * Returns true if the sales note modal contains any user-entered data.
         */
        function salesNoteModalHasData() {
            // Check customer name
            const customerName = document.getElementById('new-customer-name');
            if (customerName && customerName.value && customerName.value.trim() !== '') return true;

            // Check order date
            const salesDate = document.getElementById('new-sales-date');
            if (salesDate && salesDate.value && salesDate.value.trim() !== '') return true;

            // Check salesman
            const salesman = document.getElementById('new-salesman');
            if (salesman && salesman.value && salesman.value.trim() !== '') return true;

            // Check prepared by
            const preparedBy = document.getElementById('new-prepared-by');
            if (preparedBy && preparedBy.value && preparedBy.value.trim() !== '') return true;

            // Check checked by
            const checkedBy = document.getElementById('new-checked-by');
            if (checkedBy && checkedBy.value && checkedBy.value.trim() !== '') return true;

            // Check packed by
            const packedBy = document.getElementById('new-packed-by');
            if (packedBy && packedBy.value && packedBy.value.trim() !== '') return true;

            // Check if any items have been added
            const itemsTbody = document.getElementById('selected-items-tbody');
            if (itemsTbody && itemsTbody.querySelectorAll('tr[data-item-id], tr.item-row').length > 0) return true;
            if (itemsTbody && itemsTbody.children.length > 0) {
                const rows = Array.from(itemsTbody.children);
                const hasItems = rows.some(row => !row.querySelector('td[colspan]'));
                if (hasItems) return true;
            }

            // Check rush order checkbox
            const isRush = document.getElementById('new-is-rush');
            if (isRush && isRush.checked) return true;

            return false;
        }

        /**
         * Called when the user confirms they want to discard changes in the Sales Note modal.
         * Closes both the discard confirm modal and the add-sales-note modal, then resets form.
         */
        function confirmDiscardSalesNoteModal() {
            toggleModal('discard-sales-note-modal', false);
            toggleModal('add-sales-note-modal', false);

            // Reset customer fields
            const customerName = document.getElementById('new-customer-name');
            if (customerName) customerName.value = '';
            const customerCode = document.getElementById('new-customer-code');
            if (customerCode) customerCode.value = '';
            const customerId = document.getElementById('new-customer-id');
            ...
            // Reset text fields
            ['new-salesman', 'new-prepared-by', 'new-checked-by', 'new-packed-by'].forEach(function(id) {
            ...
            // Reset rush checkbox
            const isRush = document.getElementById('new-is-rush');
            if (isRush) isRush.checked = false;
            ...
            // Reset online mode if function exists
            const onlineToggleTrack = document.getElementById('online-toggle-track');
            const onlineToggleThumb = document.getElementById('online-toggle-thumb');
            if (onlineToggleTrack) onlineToggleTrack.classList.remove('bg-maroon');
            if (onlineToggleThumb) onlineToggleThumb.classList.remove('translate-x-4');
        ...
        window.activeEditInvoiceNumber = '';
        ...
        window.loadInvoicesForEdit = async function() {
            ...
            try {
                const url = `<?php echo e(url('/special/sales/sales-note')); ?>/${noteId}/invoices`;
                ...
                if (invoices.length === 0) {
                    ...
                    // Scenario 2: Partial note - add "Remaining Items" card
                    if (noteStatus === 'Partial' && remainingCount > 0) {
                        cardsHtml += `
        ...
        window.selectInvoiceForEdit = async function(invoiceId, invoiceNumber) {
            ...
            try {
                const url = `<?php echo e(url('/special/sales/sales-note/invoice')); ?>/${invoiceId}/items`;
                const res = await fetch(url);
                const result = await res.json();

                if (result.success) {
                    const heading = document.getElementById('edit-step3-heading');
                    if (heading) heading.innerText = 'Invoiced Items — ' + (invoiceNumber || 'Invoice');
                    ...
                    if (result.items && result.items.length > 0) {
                        result.items.forEach(item => {
                            ...
                            if ((Number(item.actual_qty) || 0) <= 0) return;
                            ...
                            row.setAttribute('data-mode', 'invoiced');
                            row.setAttribute('data-ordered-qty', item.ordered_qty ?? item.quantity ?? 0);
                            row.setAttribute('data-actual-qty', item.actual_qty ?? 0);
                            ...
                            row.innerHTML = `
                                ...
                                <td class="p-4 px-6 text-center"><input type="number" value="${displayQty}" min="0" oninput="updateEditSalesItemSubtotal(this)" onchange="updateEditSalesItemSubtotal(this)" class="item-qty w-20 px-3 py-1.5 border border-slate-200 rounded-lg text-center focus:ring-2 focus:ring-maroon/20 outline-none transition-all bg-white"><input type="hidden" class="item-actual-qty" value="${item.actual_qty ?? 0}"></td>
                                ...
                                <td class="p-4 px-6 text-right"><input type="number" value="${item.unit_price}" class="item-price w-28 px-3 py-1.5 border border-slate-200 rounded-lg text-right focus:ring-2 focus:ring-maroon/20 outline-none transition-all bg-slate-100" readonly></td>
        ...
        window.proceedEditRemainingItems = async function() {
            ...
            document.getElementById('edit-step3-heading').innerText = 'Remaining Items';
            window._editAddedProductCodes = new Set();
            window._editDeletedProductCodes = new Set();

            try {
                const url = `<?php echo e(url('/special/sales/sales-note')); ?>/${window.activeEditNoteId}/invoices`;
                const res = await fetch(url);
                const result = await res.json();
                ...
                if (result.success) {
                    ...
                    items.forEach(item => {
                        const displayQty = Number(item.remaining_qty ?? item.quantity ?? 0);
                        window._originalItems.push({ 
                            item_id: item.id, 
                            qty: displayQty, 
                            price: item.unit_price, 
                        ...
                        row.setAttribute('data-mode', 'remaining');
                        row.setAttribute('data-ordered-qty', item.ordered_qty ?? item.quantity ?? 0);
                        row.setAttribute('data-actual-qty', item.actual_qty ?? 0);
                        row.dataset.onHand = String(Number(item.on_hand ?? 0));
                        row.className = "hover:bg-slate-50 transition-colors";
                        row.innerHTML = `
                            ...
                            <td class="p-4 px-6 font-bold text-maroon product-code">${item.product_code}</td>
                            <td class="p-4 px-6 text-slate-700 product-desc">${item.description}</td>
                            <td class="p-4 px-6 text-center"><input type="number" value="${displayQty}" min="0" oninput="updateEditSalesItemSubtotal(this)" onchange="updateEditSalesItemSubtotal(this)" class="item-qty w-20 px-3 py-1.5 border border-slate-200 rounded-lg text-center focus:ring-2 focus:ring-maroon/20 outline-none transition-all"><input type="hidden" class="item-actual-qty" value="${item.actual_qty ?? 0}"></td>
                            <td class="p-4 px-6 text-center"><input type="text" value="${item.oum}" class="item-unit w-20 px-3 py-1.5 border border-slate-200 rounded-lg text-center focus:ring-2 focus:ring-maroon/20 outline-none transition-all"></td>
                            ${window.renderSalesNotePriceCodeCell ? window.renderSalesNotePriceCodeCell(item, 'updateEditSalesItemSubtotal(this)') : ''}
                            <td class="p-4 px-6 text-right"><input type="number" value="${item.unit_price}" class="item-price w-28 px-3 py-1.5 border border-slate-200 rounded-lg text-right focus:ring-2 focus:ring-maroon/20 outline-none transition-all bg-slate-100" readonly></td>
                            <td class="p-4 px-6 text-right"><input type="number" value="${item.subtotal}" class="item-subtotal w-32 px-3 py-1.5 border border-slate-200 rounded-lg text-right font-bold text-maroon bg-slate-50" readonly></td>
        ...
        window.proceedEditWithoutInvoice = async function() {
            ...
            document.getElementById('edit-step3-heading').innerText = 'Open Note';
            window._editAddedProductCodes = new Set();
            window._editDeletedProductCodes = new Set();
        ...
        window.removeEditItemRow = function(btn) {
            ...
            if (itemId && itemId !== '0') window._pendingDeletedItemIds.add(String(itemId));
        ...
        window.submitEditSalesNote = function() {
            ...
            window._pendingDeletedItemIds = window._pendingDeletedItemIds || new Set();
            let addCount = 0, updateCount = 0, totalAmount = 0;
            document.querySelectorAll('#edit-selected-items-tbody tr').forEach(row => {
                if (row.style.display === 'none') return;
                const itemId = row.getAttribute('data-item-id') || '';
                const checkbox = row.querySelector('input[type="checkbox"]');
                const isChecked = window._editOnlineMode || !checkbox || checkbox.checked;
                if (!isChecked) {
                    if (itemId) window._pendingDeletedItemIds.add(String(itemId));
                    return;
                }
                totalAmount += parseFloat(row.querySelector('.item-subtotal')?.value) || 0;
                if (!itemId) {
                    addCount++;
                    return;
                }
                ...
                if (!orig || !(orig.qty === qty && orig.price === price && (orig.price_code || '') === priceCode && orig.disc === disc && orig.bonus === bonus)) {
                    updateCount++;
                }
            });
            ...
            document.getElementById('edit-confirm-delete-count').textContent = window._pendingDeletedItemIds.size;
        ...
        window.finalizeEditSalesNote = async function() {
            ...
            document.querySelectorAll('#edit-selected-items-tbody tr').forEach(row => {
                ...
                const itemId = row.getAttribute('data-item-id') || '';
                const checkbox = row.querySelector('input[type="checkbox"]');
                if (!window._editOnlineMode && checkbox && !checkbox.checked) {
                    if (itemId) window._pendingDeletedItemIds.add(String(itemId));
                    return;
                }
                const salesOrderItemId = row.getAttribute('data-sales-order-item-id') || '';
                const qty = parseInt(row.querySelector('.item-qty').value) || 0;
                const actualQty = parseInt(row.querySelector('.item-actual-qty')?.value) || 0;
            ...
            if (!window._editOnlineMode && items.length === 0 && window._pendingDeletedItemIds.size === 0) { alert('Please select at least one item.'); return; }
            ...
            try {
                ...
                const deletedItemIds = Array.from(window._pendingDeletedItemIds).map(Number).filter(id => id > 0);
                ...
                if (result.success) {
                    ...
                    // Scenario 2/3 (Remaining Items) or Scenario 1 (Open note): No invoice selected
                    if (!window.activeEditInvoiceId) {
                        ...
                        window.location.href = window.salesRoutes.salesOrderUrl || '/special/sales/sales-order';
                        return;
                    }

                    // Scenario 2 (Edit specific invoice): redirect with edit params
                    ...
                    window.location.href = (window.salesRoutes.salesOrderUrl || '/special/sales/sales-order') + '?' + params.toString();
                } else {
...
<?php echo $__env->make('partials.special_user.special_sidebar_navbar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\hatdog\resources\views/Special_User/sales/sales-note.blade.php ENDPATH**/ ?>