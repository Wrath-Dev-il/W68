...
class PaymentHistoryService
{
    ...
    public function paymentLedger(int $paymentId): array
    {
        ...
        foreach ($invoiceRows as $invoice) {
            ...
            $invoiceDate = $this->sourceInvoiceCreatedAt((string) $invoice->source_type, (int) $invoice->source_id, $invoiceNo);
            $items = $this->invoiceItems((string) $invoice->source_type, (int) $invoice->source_id, $invoiceNo);
            $invoiceReturnTotal = 0.0;
            ...
            foreach ($items as $index => $item) {
                ...
                $invoiceReturnTotal += $returnedAmount;
            ...
            // Always show the Payment No. in Ledger, even when the monetary payment is
            // exactly 0.00. A fully returned/adjusted invoice is legitimately settled
            // with zero cash, but hiding this row made the Payment No. look missing.
            $paidAmount = (float) ($invoice->paid_amount ?? 0);
            $adjustment = (float) ($invoice->adjustment ?? 0);
            $paymentDate = $payment->payment_date ? substr((string) $payment->payment_date, 0, 10) : '';
            $paymentRemarks = trim((string) (($invoice->remarks ?? '') !== '' ? $invoice->remarks : ($payment->remarks ?? '')));

            if ($paidAmount <= 0.005) {
                if ($invoiceReturnTotal > 0.005) {
                    $settlementNote = 'Settled by Sales Return; no cash payment.';
                } elseif ($adjustment > 0.005) {
                    $settlementNote = 'Settled by Adjustment; no cash payment.';
                } else {
                    $settlementNote = 'Payment record posted with zero cash payment.';
                }
                $paymentRemarks = trim($paymentRemarks . ($paymentRemarks !== '' ? ' | ' : '') . $settlementNote);
            }
