(function () {
    ...
    async function requestChunk(connection, table, offset, knownTotal) {
        ...
        // On the first request knownTotal is null. Do not send it as JSON null,
        // otherwise Laravel Request::has('knownTotal') treats the key as present
        // and older controllers convert null to integer 0.
        if (knownTotal !== null && knownTotal !== undefined && Number.isFinite(Number(knownTotal))) {
            payload.knownTotal = Number(knownTotal);
        }
