...
window.loadSalesReturnDashboard = async function() {
    try {
        ...
        if (r.success) {
            const statMap = {
                'dash-total-returns': r.total_returns,
                'dash-total-year': r.total_year,
                'dash-total-month': r.total_month,
                'dash-online-returns': r.online_returns,
                'dash-online-year': r.online_year,
                'dash-online-month': r.online_month,
                'dash-local-returns': r.local_returns,
                'dash-local-year': r.local_year,
                'dash-local-month': r.local_month,
                'dash-return-average': r.return_average,
            };
...
function renderInvoicePagination() {
    ...
    if (info) {
        if (window._invoiceSearchTotal > 0) {
            const from = (window._invoiceSearchPage - 1) * 25 + 1;
            const to = Math.min(window._invoiceSearchPage * 25, window._invoiceSearchTotal);
